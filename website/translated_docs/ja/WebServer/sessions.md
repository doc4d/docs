...
> Webサーバーコードを 4D Server でデバッグするには、[4D Server と同じマシン上の 4D](Desktop/clientServer.md#4D-と-4D-Server-の同じマシン上での使用) を開き、サーバーに接続します。 これにより、すべてのプロセスがコオペラティブモードに切り替わり、Webサーバーコードのデバッグが可能になります。
