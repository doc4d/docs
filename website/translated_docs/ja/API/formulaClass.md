...
`Formula` コマンドは、 <!-- REF formulaClass.Formula.Summary -->*formulaExp* の式に基づいた `4D Function` オブジェクトを作成します<!-- END REF -->。 *formulaExp* can be as simple as a single value or complex, such as a project method with parameters.
...
`Formula from string` コマンドは、 <!-- REF formulaClass.Formula from string.Summary -->*formulaString* に基づいた `4D Function` オブジェクトを作成します<!-- END REF -->。  *formulaString* can be as simple as a single value or complex, such as a project method with parameters.
