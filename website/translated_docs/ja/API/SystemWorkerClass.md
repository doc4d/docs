...
| timeout          | Number  | 未定義     | プロセスが生きている場合、キルされるまでの秒数。                                                                                                                                     |
...
`.postMessage()` 関数は、 <!-- REF #SystemWorkerClass.postMessage().Summary -->外部プロセスの入力ストリーム (stdin) への書き込みをおこないます<!-- END REF -->。 In the *message* parameter, pass the text to write in *stdin*.
