...
アプリケーションビルドに関わる各パラメーター設定は XMLキーの形で、`buildApp.4DSettings` という名称のアプリケーションプロジェクトファイルに保存されます。 この XMLファイルは [プロジェクトの `Settings` フォルダー](../Project/architecture.md#settings-1) に配置されます。
