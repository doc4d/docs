...
In *vpAreaName*, pass the name of the 4D View Pro area. Si pasa un nombre que no existe, se devuelve un error.
...
| formula              | objeto   | Método de retrollamada que se lanzará cuando la exportación haya finalizado. Using a callback method is necessary when the export is asynchronous (which is the case for PDF and Excel formats) if you need some code to be executed after the export. The callback method must be used with the [`Formula`](https://doc.4d.com/4dv19/help/command/en/page1597.html) command (see below for more information).                                                                                                                                                    |
...
| includeBindingSource | Booleano | True (default) to export the current data context values as cell values in the exported object (data contexts themselves are not exported). False en caso contrario. La vinculación de celda siempre se exporta.                                                 |
...
Usted desea exportar el área, excluyendo la información de formato:
...
| sheetIndex | Integer                  | -> | Índice de la hoja para obtener el contexto de datos |
...
| comment   | Texto | Comentario correspondiente a la fórmula nombrada o al rango nombrado                                                                                            |
...
`VP Get workbook options`<!-- REF #_method_.Get workbook options. Summary -->returns an object containing all the workbook options in *vpAreaName*
<!-- END REF -->
...
| Propiedad    | Tipo         | Descripción                                                                                                                                                                                            |
| ------------ | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| copy         | Booleano     | Determines if the values, formatting and formulas of the cells in *originRange* are removed after the command executes:<ul><li>*False* (por defecto) para eliminarlos</li><li>*True* para conservarlos</li> |
...
In the optional *scope* parameter, you can designate a specific spreadsheet where *rangeName* is defined. Si se omite, se utiliza por defecto la hoja de cálculo actual. You can explicitly select the current spreadsheet or the entire workbook with the following constants:
...
| size      | texto | Define el tamaño de la fuente.<p><p>The line-height can be added to the font-size: font-size/line-height: Ex: "15pt/20pt" | un número con una de las siguientes unidades: <li>"em", "ex", "%", "px", "cm", "mm", "in", "pt", "pc", "ch", "rem", "vh", "vw", "vmin", "vmax"</li><p><p>o uno de los siguientes:<li>`vk font size large`</li><li>`vk font size larger`</li><li>`vk font size x large`</li><li>`vk font size xx large`</li><li>`vk font size small`</li><li>`vk font size smaller`</li><li>`vk font size x small`</li><li>`vk font size xx small`</li> | Sí          |
...
| options    | Entero largo | -> | Especifica lo que se pega             |
...
| ---------- | ------- | -- | -------------------------------------------- |
| vpAreaName | Texto   | -> | Nombre de objeto formulario área 4D View Pro |
| styleName  | Texto   | -> | Nombre del estilo a eliminar                 |
| scope      | Integer | -> | Alcance objetivo (por defecto = hoja actual) |
...
|                        |            | \[ ].name | Texto                | Nombre de un parámetro a mostrar en `<functionName>`.<p><p>**Note**: Parameter names must not contain space characters.                                                                                                                                                                                                                                                                                                                                                   |
...
| -------------------- | ------ | -- | ------------------------------------------- |
| rangeObj             | Objeto | -> | Objeto rango                                |
| dataContextAttribute | Texto  | -> | Nombre del atributo a vincular a *rangeObj* |
...
| innerVertical   | booleano | Estilo de la línea de borde aplicado sólo a los bordes verticales interiores.         |
...
The formulas specified by `VP SET CUSTOM FUNCTIONS` appear in a pop-up menu when the first letter of their name is entered. Ver la página de [Fórmulas y Funciones](formulas.md).
...
|                          | formula    |            | Objeto               | Objeto fórmula 4D (obligatorio). Ver el comando `Formula`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
|                          |            | \[ ].type | Número               | Tipo de parámetro. Tipos soportados:<li>`Is Boolean`</li><li>`Is date`</li><li>`Is Integer`</li><li>`Is object`</li><li>`Is real`</li><li>`Is text`</li><li>`Is time`</li>If *type* is omitted or if the default value (-1) is passed, the value is automatically sent with its type, except date or time values which are sent as an object (see [Parameters](formulas.md#parameters) section).<p><p>If *type* is `Is object`, the object has the same structure as the object returned by [VP Get value](#vp-get-value). |
...
| options    | Objeto  | -> | Opciones adicionales                             |
| sheetIndex | Integer | -> | Índice de la hoja                                |
...
In *sheetIndex*, pass the index of the sheet that will receive the data context. Si no se pasa ningún índice, el contexto se aplica a la hoja actual.
...
| pageBreak       | booleano | True para insertar un salto de página antes de la primera línea del rango, si no, false |
...
| index      | Integer | -> | Índice de la hoja a renombrar                |
...
En *index*, pase el índice de la hoja a renombrar.
...
En *index*, pase el índice de la hoja de destino. Si no se especifica ningún índice, el comando se aplica a la hoja actual.
...
| ---------- | ------ | -- | ----------------------------------------- |
| rangeObj   | Objeto | -> | Objeto rango                              |
| valueObj   | Objeto | -> | Valores de la celda y opciones de formato |
...
| format    | Texto                                    | Patrón de propiedad valor/tiempo. For information on patterns and formatting characters, please refer to the [Cell Format](configuring.md#cell-format) paragraph. |
...
| allowAutoCreateHyperlink              | booleano                | Permite la creación automática de hipervínculos en la hoja de cálculo.                                                                                                                                                                                 |
...
| cutCopyIndicatorVisible               | booleano                | Muestra un indicador al copiar o cortar el elemento seleccionado.                                                                                                                                                                                      |
...
| iterativeCalculation                  | booleano                | Activa el cálculo iterativo. See on [Grapecity's website](https://www.grapecity.com/spreadjs/docs/v14/online/calculating-iterative.html).                                                                                                              |
...
| newTabVisible                         | booleano                | Mostrar una pestaña especial para permitir a los usuarios insertar nuevas hojas.                                                                                                                                                                       |
...
| scrollbarMaxAlign                     | booleano                | La barra de desplazamiento se alinea con la última línea y columna de la hoja activa.                                                                                                                                                                  |
...
| showVerticalScrollbar                 | booleano                | Mostrar la barra de desplazamiento vertical.                                                                                                                                                                                                           |
| tabEditable                           | booleano                | The sheet tab strip can be edited.                                                                                                                                                                                                                     |
| tabNavigationVisible                  | booleano                | Mostrar la navegación por pestañas.                                                                                                                                                                                                                    |
...
| useTouchLayout                        | booleano                | Si se va a utilizar el diseño táctil para presentar el componente Spread.                                                                                                                                                                              |
