...
| .**accessTokenOAuth2**: Text<p>Text string representing OAuth 2 authorization credentials. Used only with OAUTH2 `authenticationMode`. If `accessTokenOAuth2` is used but `authenticationMode` is omitted, the OAuth 2 protocol is used (if allowed by the server). Non retourné en objet *[SMTP transporter](#smtp-transporter-object)*. | none                                                                |
...
| **password** : Text<p>User password for authentication on the server. Non retourné en objet *[SMTP transporter](#smtp-transporter-object)*.                                                                                                                                                                                               | none                                                                |
