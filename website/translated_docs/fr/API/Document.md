...
La propriété `.creationTime` retourne <!-- REF #document.creationTime.Summary -->l'heure de création du fichier<!-- END REF --> (exprimée en nombre de secondes à partir de 00:00).
...
La propriété `.exists` retourne <!-- REF #document.exists.Summary -->vrai si le fichier existe sur le disque<!-- END REF -->, sinon elle retourne faux.
...
La propriété `.extension` retourne <!-- REF #document.extension.Summary -->l'extension du nom de fichier (le cas échéant)<!-- END REF -->. Une extension commence toujours par ".". La propriété retourne une chaine vide si le nom du fichier ne contient pas d'extension.
...
La propriété `.fullName` retourne <!-- REF #document.fullName.Summary -->le nom complet du fichier, extension comprise (le cas échéant)<!-- END REF -->.
...
La propriété `.hidden` retourne <!-- REF #document.hidden.Summary -->vrai si le fichier est identifié comme "caché" au niveau du système<!-- END REF -->, sinon elle retourne faux.
...
La propriété `.isAlias` retourne <!-- REF #document.isAlias.Summary -->vrai si le fichier est un alias, un raccourci ou un lien symbolique<!-- END REF -->, sinon elle retourne faux.
...
La propriété `.isWritable` retourne <!-- REF #document.isWritable.Summary -->vrai si le fichier existe sur disque et s'il est modifiable<!-- END REF -->.
...
La propriété `.modificationDate` retourne <!-- REF #document.modificationDate.Summary -->la date de la dernière modification apportée au fichier<!-- END REF -->.
...
La propriété `.modificationTime` retourne <!-- REF #document.modificationTime.Summary -->l'heure de la dernière modification du fichier<!-- END REF --> (exprimée en nombre de secondes depuis 00:00).
...
La propriété `.name` retourne <!-- REF #document.name.Summary -->le nom du fichier, sans l'extension (le cas échéant)<!-- END REF -->.
...
La propriété `.original` retourne <!-- REF #document.original.Summary -->l'élément cible d'un alias, d'un raccourci ou d'un lien symbolique<!-- END REF -->. L'élément cible peut être :
...
La propriété `.platformPath` retourne <!-- REF #document.platformPath.Summary -->le chemin du fichier exprimé dans la syntaxe de la plate-forme<!-- END REF -->.
...
La propriété `.size` retourne <!-- REF #document.size.Summary -->la taille du fichier exprimée en octets<!-- END REF -->. Si le fichier n'existe pas sur le disque, la taille est de 0.
...
La fonction `.copyTo()` <!-- REF #document.copyTo().Summary -->copie l'objet `File` (fichier source) vers le *destinationFolder* spécifié <!-- END REF -->.
...
La fonction `.getContent()`  <!-- REF #document.getContent().Summary -->retourne un objet `4D.Blob` contenant l'intégralité du contenu d'un fichier<!-- END REF -->. Pour plus d'informations sur les BLOB, veuillez vous reporter à la section [BLOB](Concepts/dt_blob.md).
...
#### Description
La fonction `.getText()` <!-- REF #document.getText().Summary -->retourne le contenu du fichier en tant que texte <!-- END REF -->.
