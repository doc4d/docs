...
| Ergebnis  | 4D.EntitySelection | <- | References on all entities related to the Dataclass |
<!-- END REF -->
...
| settings  | Objekt             | -> | Build option: context                            |
| Ergebnis  | 4D.EntitySelection | <- | Entity selection filled from the collection      |
<!-- END REF -->
...
| settings   | Objekt          | -> | Build option: context                       |
| Ergebnis   | 4D.Entity       | <- | Entity matching the designated primary key  |
<!-- END REF -->
...
| Parameter | Typ          |    | Beschreibung               |
| --------- | ------------ |:--:| -------------------------- |
| Ergebnis  | cs.DataStore | <- | Datastore of the dataclass |
<!-- END REF -->
...
| Parameter | Typ    |    | Beschreibung                 |
| --------- | ------ | -- | ---------------------------- |
| Ergebnis  | Objekt | <- | Information on the dataclass |
<!-- END REF -->
...
| Parameter | Typ       |    | Beschreibung                      |
| --------- | --------- | -- | --------------------------------- |
| Ergebnis  | 4D.Entity | <- | New entity matching the Dataclass |
<!-- END REF -->
...
| Ergebnis  | 4D.EntitySelection | <- | New blank entity selection related to the dataclass                                                                                           |
<!-- END REF -->
...
| Ergebnis      | 4D.EntitySelection | <- | New entity selection made up of entities from dataclass meeting the search criteria specified in *queryString* or *formula* |
<!-- END REF -->
...
| attributes    | Objekt  | **Named placeholders for attribute paths** used in the *queryString* or *formula*. Attributes are expressed as property / value pairs, where property is the placeholder name inserted for an attribute path in the *queryString* or *formula* (":placeholder"), and value can be a string or a collection of strings. Each value is a path that can designate either a scalar or a related attribute of the dataclass or a property in an object field of the dataclass<p><table><tr><th>Typ</th><th>Beschreibung</th></tr><tr><td>String</td><td>attributePath expressed using the dot notation, e.g. "name" or "user.address.zipCode"</td></tr><tr><td>Collection of strings</td><td>Each string of the collection represents a level of attributePath, e.g. \["name"] or \["user","address","zipCode"]. Using a collection allows querying on attributes with names that are not compliant with dot notation, e.g. \["4Dv17.1","en/fr"]</td></tr></table>You can mix indexed placeholders (values directly passed in *value* parameters) and named placeholder values in the same query. |
...
| Parameter | Typ    |    | Beschreibung                                                               |
| --------- | ------ | -- | -------------------------------------------------------------------------- |
| settings  | Objekt | -> | Object that sets the timeout and size of the ORDA cache for the dataclass. |
<!-- END REF -->
...
| Property   | Typ            | Beschreibung        |
| ---------- | -------------- | ------------------- |
| timeout    | Lange Ganzzahl | Timeout in seconds. |
| maxEntries | Lange Ganzzahl | Number of entities. |
...
| Parameter | Typ    |    | Beschreibung                                                        |
| --------- | ------ | -- | ------------------------------------------------------------------- |
| result    | Objekt | <- | Object describing the contents of the ORDA cache for the dataclass. |
<!-- END REF -->
...
| Property   | Typ            | Beschreibung                                                              |
| ---------- | -------------- | ------------------------------------------------------------------------- |
| maxEntries | Lange Ganzzahl | Maximum number of entries collection.                                     |
| stamp      | Lange Ganzzahl | Stamp of the cache.                                                       |
| timeout    | Lange Ganzzahl | Time remaining before the new entries in the cache are marked as expired. |
...
| result      | Objekt | <- | Object that holds information on the optimization context |
<!-- END REF -->
...
| dataclassObject | Objekt                      | -> | dataclass object (e.g datastore.Employee)                     |
...
| pageLength      | Ganzzahl                    | -> | Page length of the entity selection associated to the context |
<!-- END REF -->
...
| Parameter | Typ      |    | Beschreibung                        |
| --------- | -------- | -- | ----------------------------------- |
| result    | Ganzzahl | <- | Number of entities in the dataclass |
<!-- END REF -->
#### Beschreibung
