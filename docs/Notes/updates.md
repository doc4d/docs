...
- IMAP transporter now supports mailbox event notifications using the IDLE protocol through a [notifier object](../API/IMAPTransporterClass.md#notifier), configurable via the `listener` property of [IMAP New transporter](../commands/imap-new-transporter).
