...
- **Non-blocking printing**: Starting with 4D v20 R4, each process has its own printing context (print options, current printer, etc.), thus allowing you to run multiple printing jobs simultaneously, for example using [OPEN PRINTING JOB](https://doc.4d.com/4dv20/help/command/en/page995.html). Uncheck the **Non-blocking printing** option to disable this new implementation in your project if needed. By default it is checked for 4D projects created with 4D v20 R4 and higher, and unchecked for 4D projects created with 4D versions prior to 4D v20 R4, as well as databases converted from binary mode to project mode. 
