...
- **Use legacy print rendering**: When this option is checked, forms displayed with "Classic" look on macOS or Windows are printed using the legacy screen-based rendering. You can uncheck this option (or pass the `Use legacy print rendering` selector to [`SET DATABASE PARAMETER`](../commands/set-database-parameter)) to use the 4D print transformation engine, designed to optimize the printed display of widgets on Liquid Glass and WinUI operating systems. This engine is available starting with 4D 21 R4. See [this section] for more information. 