...
- **Use legacy print rendering**: When this option is checked, forms displayed with "Classic" look on macOS or Windows are printed using a [legacy screen-based print renderer](../FormEditor/forms.md#legacy-print-renderer). You can uncheck this option to use the current print rendering engine, designed to optimize the printed display of widgets on Liquid Glass and Fluent UI interfaces. This engine is available starting with 4D 21 R3. See [this section](../FormEditor/forms.md#printing-forms) for more information. 