...
- **Non-blocking printing**: Starting with v20 R4, Each process have its own printing context (print options,current printer,etc.). When checked, this option allows running multiple printing jobs simultaneously. see [OPEN PRINTING JOB](https://doc.4d.com/4dv20/help/command/en/page995.html). By default this option is unchecked for databases created with 4D versions, as well as for the databases converted from binary mode to project mode. On the other hand, it is checked for databases created with 4D v20 R4 and higher. 