...
In this case, if you want to debug the web server code on 4D Server, you need to open a [4D on the same machine as 4D Server](Desktop/clientServer.md#using-4d-and-4d-server-on-the-same-machine) and connect to the server. With this configuration, all processes switch to cooperative mode and the web server code can be debugged. 
