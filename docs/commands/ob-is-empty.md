...
The **OB Is empty** command returns True if object is undefined or empty, and False if object is defined (initialized) and contains at least one property.object must have been created using the [`C_OBJECT`](c_object.md) command or designate a 4D object field.
