...
The `4D.OutGoingMessage` class allows you to build messages to be returned by your application functions in response to [REST requests](../REST/REST_requests.md) or HTTP requests received in [`4D.IncomingMessage`](IncomingMessageClass.md) objects thanks to a custom [HTTP request handler](../WebServer/http-request-handler.md). If the response is of type `4D.OutGoingMessage`, the REST server does not return an object but the object instance of the `OutgoingMessage` class.
