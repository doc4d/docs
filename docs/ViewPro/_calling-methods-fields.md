...
*	**Allowed**: it was explicitly declared using the [VP SET ALLOWED METHODS](method-list#vp-set-allowed-methods) command.
...
You can declare the name, type, and number of parameters through the parameters collection of the function you declared using the [VP SET ALLOWED METHODS](method-list#vp-set-allowed-methods) command. Optionally, you can control the number of parameters passed by the user through *minParams* and *maxParams* properties. Example:
...
For more information on supported incoming parameter types, please refer to the [VP SET ALLOWED METHODS](method-list#vp-set-allowed-methods) command description. 
