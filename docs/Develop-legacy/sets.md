...
Although its name does not begin with the character "$", the `UserSet` system set is a client set. So, when using [`INTERSECTION`](../commands/intersection.md), [`UNION`](../commands/union.md) and [`DIFFERENCE`](../commands/difference.md), make sure you compare `UserSet` only to client sets. 
