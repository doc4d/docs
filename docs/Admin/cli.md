...
|`--dataless`||Launches 4D in dataless mode. The following options are available:<li>If passed in the command line, dataless mode applies to all databases opened in 4D, as long as the application is not closed. In this mode, no file containing data is opened, even if specified in the command line, the `.4Dlink` file, or using commands such as `OPEN DATA FILE`.</li><li>If passed through the `.4Dlink` file, dataless mode only applies to the database specified in the `.4Dlink` file.</li>|
