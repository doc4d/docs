...
* *dbContextInfo* (objet): information concerning operations handled by the database engine. Contains the following properties:  
   ...
   * *client\_version* (string): four digits representing the version of the 4D engine of the application, as returned by the [Application version](application-version) command.  
