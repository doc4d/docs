...
* Save the project to disk, then load it for example using the [DOM Parse XML source](dom-parse-xml-source) command, and pass its reference in the *project* parameter.
...
**Note:** Refer to the [EXPORT DATA](export-data) command for an example concerning the definition of an empty project. 
