...
**CUT NAMED SELECTION** is more memory efficient than [COPY NAMED SELECTION](copy-named-selection). With [COPY NAMED SELECTION](copy-named-selection), 4 bytes times the number of selected records is duplicated in memory. With **CUT NAMED SELECTION**, only the reference to the list is moved.
