...
<!--REF #_command_.DELETE USER.Summary-->The **DELETE USER** command deletes the user whose unique user ID number you pass in *userID*.<!-- END REF--> You must pass a valid user ID number returned by the [GET USER LIST](get-user-list) command. 
...
Deleted user names no longer appear in the Users editor displayed when you call [EDIT ACCESS](edit-access), nor in the Design mode. Note that the numbers for deleted users can be reassigned when new user accounts are created.
