...
**Note:** The [SQL EXPORT DATABASE](sql-export-database) and [SQL EXPORT SELECTION](sql-export-selection) commands automatically generate this script file.
...
If the 4D log file is activated (via the selectors 28 or 45 of the [SET DATABASE PARAMETER](set-database-parameter) command), each SQL command executed will generate an entry with the following information:
