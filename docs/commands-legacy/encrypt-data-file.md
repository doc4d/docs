...
* **To encrypt the database for the first time**, you only need to pass the *newPassPhrase* or *newDataKey* parameter (The *curPassPhrase* or *curDataKey* parameters must not be provided):  
   ...
   * *newDataKey*: Object (with *encodedKey* property) that contains a new data encryption key. This key should have been generated with the [New data key](new-data-key) command.  
...
(\*) Warning returned at the verification step (see [VERIFY DATA FILE](verify-data-file) command).
