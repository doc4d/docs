...
Once you have created and opened a document, you can write and read the document using the [RECEIVE PACKET](receive-packet) and [SEND PACKET](send-packet) commands that you can combine with the [Get document position](get-document-position) and [SET DOCUMENT POSITION](set-document-position) commands in order to directly access any part of the document.
