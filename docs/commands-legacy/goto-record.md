...
<!--REF #_command_.GOTO RECORD.Summary-->**GOTO RECORD** selects the specified record of *aTable* as the current record.<!-- END REF--> The *record* parameter is the number returned by the [Record number](record-number) function. After executing this command, the record is the only record in the selection.
...
See the example for [Record number](record-number).
