...
If the windows to which the command is applied were not previously maximized (manually or using [MAXIMIZE WINDOW](maximize-window)), or if the window type does not include a zoom box, the command has no effect. For more information on window types, refer to the *Window Types (compatibility)* section. 
