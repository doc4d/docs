...
In the *name* and *password* parameters, you pass the id information required (user name and password). This information is encoded and added to the next HTTP request sent using the [HTTP Request](http-request) or [HTTP Get](http-get) command, so you need to call the **HTTP AUTHENTICATE** command before each HTTP request.
