...
When you apply [SELECTION TO ARRAY](selection-to-array) or [SELECTION RANGE TO ARRAY](selection-range-to-array) to a Time type field, note that they only create a Time type array if the array has not already been defined as another type, such as Longint for example. 
