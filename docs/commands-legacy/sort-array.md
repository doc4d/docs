...
If more than one array is specified, the arrays are sorted following the sort order of the first array; no multi-level sorting is performed here. Instead you can use the [MULTI SORT ARRAY](multi-sort-array) command when you want to sort synchronized arrays.
