...
**Note:** **Modified** always returns [True](true.md "True") after the execution of the [PUSH RECORD](push-record) and [POP RECORD](pop-record) commands.

In all cases, use the [Old](old) command to detect whether the field value has actually been changed.

**Note:** Although **Modified** can be applied to any type of field, if you use it in combination with the [Old](old) command, be aware of the restrictions that apply to the [Old](old) command. For details, see the description of the [Old](old) command.
