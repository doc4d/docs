...
1. Call [GET LIST ITEM](get-list-item) on the parent item to get the list reference of the sublist.
2. Call [SET LIST ITEM](set-list-item) on the parent item to detach the sublist from the list item before clearing it.
3. Call **CLEAR LIST** to clear the sublist whose reference number you obtained with [GET LIST ITEM](get-list-item).

## Example 1 

Within a clean-up routine that clears all objects and data that you no longer need (i.e., when a window is closed and a form unloaded), you may end up clearing a hierarchical list that may have already been cleared, depending on the user actions within the form. Use [Is a list](is-a-list) to clear the list only if necessary:
...
See example for the [Load list](load-list) command.

## Example 3 

See example for the [BLOB to list](blob-to-list) command.
