...
If *name* was created using the [CUT NAMED SELECTION](cut-named-selection) command and then manipulated using the [USE NAMED SELECTION](use-named-selection) command, *name* no longer exists in memory. In this case, the **CLEAR NAMED SELECTION** command does not need to be used.
