...
<!--REF #_command_.On Backup Startup database method.Summary-->The **On Backup Startup database method** is called every time a database backup is about to start (manual backup, scheduled automatic backup, or using the [BACKUP](../commands/backup.md) command).<!-- END REF-->   
...
* If $0 # 0, the backup is not authorized. The operation is cancelled and an error is returned. You can get the error using the [BACKUP INFO](../commands/backup-info.md) command.
