...
In the context of list boxes based on selections of records, **LISTBOX INSERT COLUMN** can be used to insert simple elements (fields or variables). If you want to handle more complex expressions (such as formulas or methods), you must use the [LISTBOX INSERT COLUMN FORMULA](listbox-insert-column-formula) command.   
Collection or Entity selection type list boxes are also supported, however since the *colName* parameter does not accept expressions, you must use the [LISTBOX SET COLUMN FORMULA](listbox-set-column-formula) command to assign the data source. It is more accurate to use the [LISTBOX INSERT COLUMN FORMULA](listbox-insert-column-formula) command in this case. 
...
[LISTBOX INSERT COLUMN FORMULA](listbox-insert-column-formula)  
