...
The following code illustrates the various possibilities offered by the **GET FIELD RELATION**, [GET AUTOMATIC RELATIONS](get-automatic-relations) and [SET FIELD RELATION](set-field-relation), [SET AUTOMATIC RELATIONS](set-automatic-relations) commands along with their effects:
