...
**Note:** When you write variables to documents with **SAVE VARIABLES**, 4D uses an internal data format. You can retrieve the variables only with the [LOAD VARIABLES](load-variables) command. Do not use [RECEIVE PACKET](receive-packet) or [RECEIVE VARIABLE](receive-variable) to read a document created by **SAVE VARIABLES**.
