...
* If the current record is unloaded from memory before calling **DELETE RECORD** (for example, subsequent to an [UNLOAD RECORD](unload-record)), the current selection of *table* is empty after the deletion occurs.
* The **DELETE RECORD** command does nothing when the table is in [READ ONLY](read-only) mode, regardless of whether the record to be deleted is locked or not.
