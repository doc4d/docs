...
In the *name* and *password* parameters, pass the required identification information (user name and password). This information will be encoded and added to the HTTP request sent to the Web Service using the [WEB SERVICE CALL](web-service-call) command. It is thus necessary to call the **WEB SERVICE AUTHENTICATE** command before calling the [WEB SERVICE CALL](web-service-call) command.  

The optional *authMethod* parameter indicate the authentication method to be used for the next call to the [WEB SERVICE CALL](web-service-call) command. You can pass one of the following values:
...
By default, the authentication information is reset to zero after each request. Therefore, you must use the **WEB SERVICE AUTHENTICATE** command before each [WEB SERVICE CALL](web-service-call) command. It is nevertheless possible to keep this information temporarily using an option of the [WEB SERVICE SET OPTION](web-service-set-option) command. In this case, it is not necessary to execute the **WEB SERVICE AUTHENTICATE** command before each [WEB SERVICE CALL](web-service-call) command.

If authentication fails, the SOAP server returns an error that you can identify using the [WEB SERVICE Get info](web-service-get-info) command.
