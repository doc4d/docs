...
* if "Color Scheme" set to "inherited", a call to the [SET APPLICATION COLOR SCHEME](set-application-color-scheme) command;
* if [SET APPLICATION COLOR SCHEME](set-application-color-scheme) not called or called with "inherited" parameter value, the Settings (host database Settings in case of a component);
* if Settings set to "inherited", the OS user preferences.

Please refer to the [SET APPLICATION COLOR SCHEME](set-application-color-scheme) command for details about color scheme names. 
