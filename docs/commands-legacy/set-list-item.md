...
You can pass a reference number in *itemRef*. If there is no item with the item reference number you passed, the command does nothing. You can optionally pass *0* in *itemRef* to designate the last item added to the list using [APPEND TO LIST](append-to-list).
...
If you work with item reference numbers, build a list in which the items have unique reference numbers, otherwise you will not be able to distinguish the items. For more information, refer to the [*Hierarchical List form object*](../FormObjects/list_overview) section.
...
To detach a sublist already attached to the item, pass *0* (zero) in *sublist*. In this case, it is a good idea to have previously obtained the reference number of that list using [APPEND TO LIST](append-to-list), so you can later delete the sublist using [CLEAR LIST](clear-list), if you no longer need it.
...
See example for the [GET LIST ITEM](get-list-item) command.
