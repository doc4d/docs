...
<!--REF #_command_.PAUSE PROCESS.Summary-->**PAUSE PROCESS** suspends the execution of *process* until it is reactivated by the [RESUME PROCESS](resume-process) command.<!-- END REF--> During this period, *process* does not take any time on your machine. Even though a process may be paused, the process is still in memory.

If *process* is already paused, **PAUSE PROCESS** does nothing. If the process has been delayed using the [DELAY PROCESS](delay-process) command, the process is paused. [RESUME PROCESS](resume-process) resumes the process immediately.
