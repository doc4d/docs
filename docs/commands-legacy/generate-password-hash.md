...
**Reminder**: Multiple passes of the same password will result in different hashed strings. This is a standard behavior for algorithms such as bcrypt, since the best practice is to create a new, random salt for every hash. Refer to the [Verify password hash](verify-password-hash) description for an example of how to check the passwords.
