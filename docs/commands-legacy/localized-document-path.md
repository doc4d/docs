...
**Note:** The current language is either set automatically by 4D according to the contents of the **Resources** folder (see the [Get database localization](get-database-localization) command), or via the [SET DATABASE LOCALIZATION](set-database-localization) command).
