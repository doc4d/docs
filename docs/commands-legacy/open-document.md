...
If a document is open, **Open document** initially sets the file position at the beginning of the document while [Append document](append-document) sets it at the end of the document.

Once you have opened a document, you can read and write in the document using the [RECEIVE PACKET](receive-packet) and [SEND PACKET](send-packet) commands that you can combine with the [Get document position](get-document-position) and [SET DOCUMENT POSITION](set-document-position) commands in order to directly access any part of the document.

Do not forget to eventually call [CLOSE DOCUMENT](close-document) for the document.
