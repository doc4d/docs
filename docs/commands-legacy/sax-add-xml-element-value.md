...
In *data*, you can either pass a character string directly, or a 4D variable. The variable contents will be converted into text before being included in the XML document. If *data* is evaluated to *undefined*, 4D uses an empty string. If you want to encode the contents of *data*, you must use the [BASE64 ENCODE](base64-encode) command. In this case, of course, you must passs a BLOB in *data*.

By default, the command encodes special characters (< > ” ’...) contained in the *data* parameter unless you have disabled this mechanism for the current process using the [XML SET OPTIONS](xml-set-options) command by passing the XML raw data value to the XML string encoding option. For example:  
