...
<!--REF #_command_.EXPAND BLOB.Summary-->The **EXPAND BLOB** command expands the BLOB *blob* that was previously compressed using the [COMPRESS BLOB](compress-blob) command*.<!-- END REF-->*
...
To check if a BLOB has been compressed, use the [BLOB PROPERTIES](blob-properties) command.
