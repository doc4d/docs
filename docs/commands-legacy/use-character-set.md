...
<!--REF #_command_.USE CHARACTER SET.Summary-->**USE CHARACTER SET** modifies the character set used by 4D during data transfer between the database and a document or a serial port for the current process.<!-- END REF--> Transfer operations include the import and export of text, DIF, and SYLK files. A character map also works on data transferred with [SEND PACKET](send-packet), [RECEIVE PACKET](receive-packet) (for text type packets) and [RECEIVE BUFFER](receive-buffer). It has no effect on transfers of data done with [SEND RECORD](send-record), [SEND VARIABLE](send-variable), [RECEIVE RECORD](receive-record), [SEND PACKET](send-packet), [RECEIVE PACKET](receive-packet) (for BLOB type packets) and [RECEIVE VARIABLE](receive-variable). 
