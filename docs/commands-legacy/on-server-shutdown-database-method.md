...
To close the current database on the server, you can select the **Close Database...**  menu command on the server. You can also choose the **Quit** command or call the [QUIT 4D](quit-4d) command within a stored procedure executed on the server.
...
If you want code to be executed automatically on a client machine when a remote 4D stops connecting to the server, use the [Semaphore](semaphore).
