...
**Note:** If you want to customize the icon of each item in the list, use the command [SET LIST ITEM PROPERTIES](set-list-item-properties).
...
* by programming, using the [SELECT LIST ITEMS BY POSITION](select-list-items-by-position) and [SELECT LIST ITEMS BY REFERENCE](select-list-items-by-reference) commands.  
