...
| contextAttributes | Object        | Object containing information similar to the [LOCKED BY](locked-by) command but applied to the record, the difference being that **Locked records info** returns the user name defined in the system and not that of the 4D user, as well as additional information (see below). |
...
| user4d\_alias                                                                              | Text        | User alias defined with [SET USER ALIAS](set-user-alias), otherwise user name in the 4D database directory |
