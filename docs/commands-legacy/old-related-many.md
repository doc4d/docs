...
<!--REF #_command_.OLD RELATED MANY.Summary-->**OLD RELATED MANY** operates the same way [RELATE MANY](relate-many) does, except that **OLD RELATED MANY** uses the old value in the one field to establish the relation.<!-- END REF-->**OLD RELATED MANY** uses the old value of the many field as returned by the [Old](old) function. For more information, see the description of the [Old](old) command.
