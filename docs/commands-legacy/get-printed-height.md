...
The value returned will be included between 0 (the top edge of the page) and the overall height returned by the [GET PRINTABLE AREA](get-printable-area) command (the maximum size of the printable area). 
...
The right and left printable margins, unlike the top and bottom margins (which may be defined using the [SET PRINTABLE MARGIN](set-printable-margin) command), do not influence the value returned.
