...
* If you do not pass the *\** parameter, **FORM GET ENTRY ORDER** returns the entry order as it was previously declared with the [FORM SET ENTRY ORDER](form-set-entry-order) command. You can omit or pass the *pageNumber* parameter:  
...
In both cases, if the [FORM SET ENTRY ORDER](form-set-entry-order) command was not previously called for the current form, the *objectNames* array is returned empty.
* If you pass the *\** as parameter, **FORM GET ENTRY ORDER** returns the actual entry order of the current page, i.e. the *objectNames* array only contains **valid** object names (for more information on valid objects, please refer to the [FORM SET ENTRY ORDER](form-set-entry-order) command description). The actual form entry order can be:  
   ...
   * or the entry order set by a call to the [FORM SET ENTRY ORDER](form-set-entry-order) command in the current process, if it has been used.  
