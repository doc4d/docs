...
**WARNING:** If you use a BLOB for storing lists, you must later use the command [BLOB to list](blob-to-list) for reading back the contents of the BLOB, because lists are stored in BLOBs using a 4D internal format.
...
**Note regarding Platform Independence:** LIST TO BLOB and [BLOB to list](blob-to-list) use a 4D internal format for handling lists stored in BLOBs. As a benefit, you do not need to worry about byte swapping between platforms when using these two commands. In other words, a BLOB created on Windows using those commands can be reused on Macintosh, and vice-versa.
...
See example for the command [BLOB to list](blob-to-list).
