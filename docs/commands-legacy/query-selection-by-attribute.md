...
<!--REF #_command_.QUERY SELECTION BY ATTRIBUTE.Summary-->**QUERY SELECTION BY ATTRIBUTE** works and performs the same actions as [QUERY BY ATTRIBUTE](query-by-attribute).<!-- END REF--> The difference between these two commands is the scope of the query:

* [QUERY BY ATTRIBUTE](query-by-attribute) looks for records among all the records in the table.
...
For more information, see the description of the [QUERY BY ATTRIBUTE](query-by-attribute) command.

The **QUERY SELECTION BY ATTRIBUTE** command is useful when a query cannot be defined using a combination of [QUERY BY ATTRIBUTE](query-by-attribute) (and even [QUERY](query)) calls joined with the \* parameter. Typically, this is the case when you want to query a current selection that does not result from a previous query, but from a command such as [USE SET](use-set). 
