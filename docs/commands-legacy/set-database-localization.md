...
The current language of the database lets you specify the .lproj folder where the program will look for the localized elements of the application (text and pictures). By default, 4D automatically determines the current language according to the contents of the **Resources** folder and the system environment (see the description of the [Get database localization](get-database-localization) command). SET DATABASE LOCALIZATION can be used to modify the default current language. 
