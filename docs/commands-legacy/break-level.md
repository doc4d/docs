...
<!--REF #_command_.BREAK LEVEL.Summary-->**BREAK LEVEL** specifies the number of break levels in a report performed using [PRINT SELECTION](print-selection).<!-- END REF-->

You **must** execute **BREAK LEVEL** and [ACCUMULATE](accumulate) before every report for which you want to do break processing. These commands activate break processing for a report. See the explanation for the [Subtotal](subtotal) command.
...
The following example prints a report with two break levels. The selection is sorted on four levels, but the **BREAK LEVEL** command specifies to break on only two levels. One field is accumulated with the [ACCUMULATE](accumulate) command:
