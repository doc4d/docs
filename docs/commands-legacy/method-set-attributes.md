...
Method attributes must be set using the [OB SET](ob-set) or [OB SET ARRAY](ob-set-array) commands, with True or False values for Boolean attributes, or specific values for extended attributes. Only attributes that are present in the *attributes* parameter will be updated in the method attributes.
