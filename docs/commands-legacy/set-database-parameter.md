...
You want to export data in JSON that contains a converted 4D date. Note that conversion occurs when the date is saved in the object, so you must call the [SET DATABASE PARAMETER](set-database-parameter) command before calling [OB SET](ob-set): 
...
[Get database parameter](get-database-parameter)  
[LOG EVENT](log-event)  
