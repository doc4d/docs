...
Once the command is executed, the *alias* is used wherever the current 4D user account was returned or displayed, in particular in the [Current user](current-user), [Process activity](../commands/process-activity) and [LOCKED BY](locked-by) commands, or the [ORDER BY FORMULA](order-by-formula) of the 4D Server administration window. In client/server environments, this command must be executed on remote applications so that each user is identified on the server, for example through the [Current user](current-user) command, even if all remote applications use the same 4D user account.

To remove the *alias* for the current 4D user, call **SET USER ALIAS**("").

**Note:** Calling the [CHANGE CURRENT USER](change-current-user) command resets the alias name for the current user. 
