...
<!--REF #_command_.LDAP LOGIN.Summary-->The **LDAP LOGIN** command opens a read-only connection to the LDAP server specified in the *url* parameter with the *login* and *password* identifiers provided.<!-- END REF--> If accepted by the server, this connection will be used for any LDAP searches executed subsequently in the current process until the [LDAP LOGOUT](ldap-logout) command is executed (or until the process is closed). 
...
Do not forget to call the [LDAP LOGOUT](ldap-logout) command when the connection to the LDAP server is no longer necessary. 
