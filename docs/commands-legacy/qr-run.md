...
<!--REF #_command_.QR RUN.Summary-->The **QR RUN** command executes the report *area* whose reference was passed as parameter with the Quick Report current settings, including the output type.<!-- END REF--> You can use the [QR SET DESTINATION](qr-set-destination) command to modify the output type.

The report is executed on the table to which the area belongs. When *area* designates an offscreen area, it is necessary to specify the table to be used via the [QR SET REPORT TABLE](qr-set-report-table) command.
