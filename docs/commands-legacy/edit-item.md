...
* List forms displayed using the [DISPLAY SELECTION](display-selection) or [MODIFY SELECTION](modify-selection) commands.

If the command is used with an enterable object that is not a list, it then acts the same as the [GOTO OBJECT](goto-object) command.   
