...
RECEIVE BUFFER is different from [RECEIVE PACKET](receive-packet) in that it takes whatever is in the buffer and then immediately returns. [RECEIVE PACKET](receive-packet) waits until it finds a specific character or until a given number of characters are in the buffer.
...
Note that access to the interprocess *◊vtBuffer* variable should be protected by a semaphore, so that processes will not conflict. See the command [Semaphore](semaphore) for more information.
