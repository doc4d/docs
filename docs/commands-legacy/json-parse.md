...
<!--REF #_command_.JSON Parse.Summary-->The **JSON Parse** command parses the contents of a JSON-formatted string and extracts values that you can store in a 4D field or variable.<!-- END REF--> This command deserializes JSON data; it performs the opposite action of the [JSON Stringify](json-stringify) command.
...
**Note:** If you use pointers, you must call the [JSON Stringify](json-stringify) command before calling **JSON Parse**. 
...
This example shows the combined use of the [JSON Stringify](json-stringify) and **JSON Parse** commands:
