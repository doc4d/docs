...
<!--REF #_command_.GET TABLE TITLES.Summary-->The **GET TABLE TITLES** command fills the *tableTitles* and *tableNums* arrays with the names and numbers of database tables defined in the Structure window or using the [SET TABLE TITLES](set-table-titles) command.<!-- END REF--> The contents of these two arrays are synchronized.

If the [SET TABLE TITLES](set-table-titles) command is called during the session, **GET TABLE TITLES** only returns the “modified” names and table numbers defined using this command. 
