...
You can use the "length" virtual property with this command. It is automatically available for all attributes of the array type, and provides the size of the array, i.e. the number of elements it contains. This property is designed to be used in queries (see [QUERY BY ATTRIBUTE](query-by-attribute)). You can also use it with the **DISTINCT ATTRIBUTE VALUES** command to get the different array sizes for an attribute.
