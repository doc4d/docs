...
* if you pass a pointer, it is kept as is; it is evaluated using the [JSON Stringify](json-stringify) command.
...
You want to export data in JSON that contains a 4D date that you want to be converted into a string without time zone information. Note that conversion occurs when the date is saved in the object, so you must call the [SET DATABASE PARAMETER](set-database-parameter) command before calling [OB SET](ob-set): 
