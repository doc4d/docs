...
<!--REF #_command_.GRAPH SETTINGS.Summary-->GRAPH SETTINGS changes the graph settings for graph displayed in a form.<!-- END REF--> The graph must have already been defined using the [GRAPH](graph) command. GRAPH SETTINGS has no effect on a pie chart. This command must be called in the same process as the form.

**Note:** You must not call this command if the graph was generated using the [GRAPH](graph) command with the [Object](# "Data structured as a native 4D object") type *graphSettings* parameter. Refer to the description of the [GRAPH](graph) command for more information.
...
See example for the [GRAPH](graph) command.
