...
* a remote 4D starts a global process (whose name does not begin with "$") which requires the creation of a cooperative process on the server **(\*)**. This process can be created using the [New process](../commands/new-process.md) command, a menu command or using the Execute Method dialog box.
