...
<!--REF #_command_.QR EXECUTE COMMAND.Summary-->The **QR EXECUTE COMMAND** command executes the toolbar button command whose reference was passed in *command*.<!-- END REF--> The most common use for this command is to execute a command selected by the user and intercepted in your code via the [QR ON COMMAND](qr-on-command) command.
...
| -------------------- | ----- | ---------------------------------------------- |
| qr cmd generate      | 2008  | Use of command [QR RUN](qr-run) recommended |
| qr cmd open          | 2001  |                                                |
