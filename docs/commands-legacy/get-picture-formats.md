...
The codec IDs returned are established by 4D in exactly the same way as for the [PICTURE CODEC LIST](picture-codec-list) command. They can be returned in the following forms:
...
* 4-character QuickTime codes may be returned in databases where the QuickTime support compatibility option has been set (using the [SET DATABASE PARAMETER](set-database-parameter) command). However, QuickTime is no longer supported in 4D and we do not recommend using QuickTime codecs.
