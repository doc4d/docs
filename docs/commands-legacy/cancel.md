...
* cancel a new or modified record, for which data entry has been initiated using [ADD RECORD](add-record) or [MODIFY RECORD](modify-record).
* cancel a form displayed with the [DIALOG](../commands/dialog) command.
* exit a form displaying a selection of records, using [DISPLAY SELECTION](display-selection) or [MODIFY SELECTION](modify-selection).
...
It is also often used in the optional close box method for the [Open window](../commands/open-window) command. If there is a Control-menu box on a window, [ACCEPT](accept) or **CANCEL** can be called, in the method to be executed, when the Control-menu box is double-clicked or the **Close** menu command is chosen.
...
**Note:** This operation differs from that of the [PAGE BREAK](page-break)(\*) command that cancels ALL the forms waiting to be printed.

### Headless mode 

The **CANCEL** command is allowed in headless mode, in the context of offscreen areas created by *VP Run offscreen area* or [WA Run offscreen area](wa-run-offscreen-area). 

## Example 

Refer to the example of the [SET PRINT MARKER](set-print-marker) command.

## System variables and sets 

When the [CANCEL](cancel) command is executed (form or printing cancelled), the system variable OK is set to 0.
