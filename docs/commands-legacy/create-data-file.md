...
The general functioning of this command is identical to that of the [OPEN DATA FILE](open-data-file) command; the only difference is that the new data file set by the *accessPath* parameter is created just after the structure is re-opened.
...
**4D Server:** Beginning with 4D v13, this command can be executed with 4D Server. In this context, it performs an internal call to [QUIT 4D](quit-4d) on the server (which causes a dialog box to appear on each remote machine, indicating that the server is in the process of quitting) before creating the designated file. 
