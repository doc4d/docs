...
* If you do not pass the optional *menu* parameter, the menu is created blank. You must build and manage it using the [RELEASE MENU](release-menu), [SET MENU ITEM](set-menu-item), etc. commands.
...
A menu created by this command can be used as the menu bar using the [SET MENU BAR](set-menu-bar) command.

When you no longer need the menu created by [Create menu](create-menu), remember to call the [RELEASE MENU](release-menu) command in order to free up the memory being used. 
