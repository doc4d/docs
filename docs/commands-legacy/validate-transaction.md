...
Note that when OK is set to 0, the transaction is automatically cancelled internally (equivalent to [CANCEL TRANSACTION](cancel-transaction)). Consequently, you must not explicitly call [CANCEL TRANSACTION](cancel-transaction) if OK=0, particularly in the context of nested transactions, because the cancellation will then be applied to the higher level transaction. 
