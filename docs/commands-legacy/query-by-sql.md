...
The QUERY BY SQL command is similar to the [QUERY BY FORMULA](query-by-formula) command. It looks for records in the specified table. It changes the current selection of *aTable* for the current process and makes the first record of the new selection the current record.
...
The *sqlFormula* expression may be simple, such as comparing a field (column) to a value; or it may be complex, such as performing a calculation. Like [QUERY BY FORMULA](query-by-formula), QUERY BY SQL is able to evaluate information in related tables (see example 4). *sqlFormula* must be a valid SQL statement that is compliant with the SQL-2 standard and with respect to the limitations of the current SQL implementation of 4D. For more information about SQL support in 4D, refer to the *4D SQL Reference* manual.

The *sqlFormula* parameter can use references to 4D expressions. The syntax to use is the same as for the integrated SQL commands or the code included between the [Begin SQL](begin-sql)/[End SQL](end-sql) tags, i.e.: *<<MyVar>>* or *:MyVar*.

**Note:** This command is compatible with the [SET QUERY LIMIT](set-query-limit) and [SET QUERY DESTINATION](set-query-destination) commands.
...
Using the [QUERY BY FORMULA](query-by-formula) command, you can write:
...
**Note:** QUERY BY SQL handles One-to-Many and Many-to-Many relations differently than [QUERY BY FORMULA](query-by-formula).
...
Using the [QUERY BY FORMULA](query-by-formula) command, you could write:
