...
If you work with item reference numbers, build a list in which items have unique reference numbers, otherwise you will not be able to distinguish the items. For more information, refer to the description of the command [APPEND TO LIST](append-to-list).
...
* *icon* returns the picture assigned to the item, if any.  
   ...
   **Note:** You can retrieve, in a picture variable, the icon associated with an item using the [GET LIST ITEM ICON](get-list-item-icon) command*.*
* *color* returns the color of the text of the item specified.

For details about these properties, see the description of the command [SET LIST ITEM PROPERTIES](set-list-item-properties).
