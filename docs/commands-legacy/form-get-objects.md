...
**Note:** The command does not include list box parts. To parse a form for the list box objects, [LISTBOX GET OBJECTS](listbox-get-objects) or [LISTBOX GET ARRAYS](listbox-get-arrays) should be used (see example below for a combination of [FORM GET OBJECTS](form-get-objects), [OBJECT Get type](object-get-type) and [LISTBOX GET OBJECTS](listbox-get-objects)).
...
Pass the name of the pointer array that already contains pointers to variables or fields associated with objects in the optional *variablesArray* parameter. If an object does not have an associated variable, the pointer [Is nil pointer](is-nil-pointer) is returned. If there is a “subform” type object, a pointer to the table of the subform is returned.
