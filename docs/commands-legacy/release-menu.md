...
Any sub-menus used by *menu* are not removed if they were created directly using the [Create menu](create-menu) command. In this case, you must remove them individually (see the rule mentioned above). However, if the submenus come from the duplication of an existing menu, do not call [RELEASE MENU](release-menu) with them because 4D will erase them automatically.
