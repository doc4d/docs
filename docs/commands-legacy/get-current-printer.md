...
To obtain the list of available printers as well as additional information, use the [PRINTERS LIST](printers-list) command. To modify the current printer, use the [SET CURRENT PRINTER](set-current-printer) command.

**Note:** When the Generic PDF driver constant is used with [SET CURRENT PRINTER](set-current-printer), [Get current printer](get-current-printer) returns "\_4d\_pdf\_printer" or the actual name of the PDF driver.
