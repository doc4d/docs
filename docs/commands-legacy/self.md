...
<!--REF #_command_.Self.Summary-->This command is kept only for compatibility reasons.<!-- END REF--> Starting with version 12 of 4D, it is recommended to use the [OBJECT Get pointer](object-get-pointer) command.
...
If **Self** is called out of context, it returns a [Is nil pointer](is-nil-pointer) pointer (*\->\[\]*).
...
See the example for the [RESOLVE POINTER](resolve-pointer) command.
