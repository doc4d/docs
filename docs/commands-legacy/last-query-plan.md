...
This command returns a significant value if the [DESCRIBE QUERY EXECUTION](describe-query-execution) command has been executed during the session. 

The description of the last query plan can be compared to the description of the actual path of the last query (obtained using the [Last query path](last-query-path) command) for optimization purposes.
