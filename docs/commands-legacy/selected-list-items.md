...
**Note:** If you use the @ character in the name of the list object and the form contains several lists that match with this name, the [Selected list items](selected-list-items) command will only apply to the first object whose name corresponds. 
...
**Note:** In the event of multiple selections, the command returns the position or reference of the first item that was selected in *list*, either by the user (manual selections) or by the [SELECT LIST ITEMS BY POSITION](select-list-items-by-position) or [SELECT LIST ITEMS BY REFERENCE](select-list-items-by-reference) commands (programmed selection). 
