...
If the menu item specified by *menu* and *menuItem* is itself a menu managed by reference and created, for example, using the [Create menu](create-menu) command, **DELETE MENU ITEM** will only delete the instance of the *menuItem* in *menu*. The submenu referenced by the *menuItem* will continue to exist in memory. You must use the [RELEASE MENU](release-menu) command in order to definitively delete a menu that is managed by reference. 

This command also works with a menu bar created using the [Create menu](create-menu) command and installed with the [SET MENU BAR](set-menu-bar) command.
