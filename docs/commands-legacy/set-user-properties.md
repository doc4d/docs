...
If you are changing the properties of an existing user account, you must pass a valid user ID number returned by the [GET USER LIST](get-user-list) command. If the user account does not exist or has been deleted, the error -9979 is generated. You can catch this error with an error-handling method installed using [ON ERR CALL](on-err-call.md). Otherwise, you can call [Is user deleted](is-user-deleted) to test the user account before calling Set user properties.
...
If you do not want to change all the properties of the user (aside from the memberships, see below), first call [GET USER PROPERTIES](get-user-properties) and pass the returned values for the properties you want to leave unchanged.
