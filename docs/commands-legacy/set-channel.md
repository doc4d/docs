...
**Historical Note:** This command was originally the first 4D command used for working with serial ports and documents on disks. Since that time, new commands have been added. Today, you will typically work with documents on disk using the commands [Open document](open-document), [Create document](create-document) and [Append document](append-document). With these commands, you can read and write characters to and from documents using [Create document](create-document) or [RECEIVE PACKET](receive-packet) (these commands work with **SET CHANNEL**, too). However, if you want to use the commands [SEND VARIABLE](send-variable), [RECEIVE VARIABLE](receive-variable), [SEND RECORD](send-record) and [RECEIVE RECORD](receive-record), you must use SET CHANNEL to access the document on disk.
...
The first form of the **SET CHANNEL** command opens a serial port, setting the protocol and other port information. Data can be sent with [SEND PACKET](send-packet), [SEND RECORD](send-record) or [SEND VARIABLE](send-variable), and received with [RECEIVE BUFFER](receive-buffer), [RECEIVE PACKET](receive-packet), [RECEIVE VARIABLE](receive-variable) or [RECEIVE RECORD](receive-record).
...
See examples for the [RECEIVE BUFFER](receive-buffer), [SET TIMEOUT](set-timeout) and [RECEIVE RECORD](receive-record) commands.
