...
<!--REF #_command_.RECEIVE VARIABLE.Summary-->**RECEIVE VARIABLE** receives *variable*, which was previously sent by [SEND VARIABLE](send-variable) from the document or serial port previously opened by [SET CHANNEL](set-channel).<!-- END REF--> 
...
1. If you receive a variable from a document using this command, the document must have been opened using the [SET CHANNEL](set-channel) command. You cannot use **RECEIVE VARIABLE** with a document opened with [Open document](open-document), [Create document](create-document) or [Append document](append-document).
