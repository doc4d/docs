...
Style, color, background color or row control arrays can be associated with array type list boxes or (except for row control arrays) with the columns of array type list boxes, using the Property list in Design mode or using the [LISTBOX SET ARRAY](listbox-set-array) command. 
...
* [Is nil pointer](is-nil-pointer) if no array for the requested property is associated with the column or the list box.
* a pointer to the array of the requested property, defined by the user.
* a pointer to the array of the requested property, defined dynamically when calling the [LISTBOX SET ROW COLOR](listbox-set-row-color) or [LISTBOX SET ROW FONT STYLE](listbox-set-row-font-style) command.
