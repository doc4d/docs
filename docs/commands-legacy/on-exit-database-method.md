...
* A call to the [QUIT 4D](quit-4d) command is issued
* A 4D Plug-in issues a call to the [QUIT 4D](quit-4d) entry point
...
* awakes processes that have been delayed by [DELAY PROCESS](delay-process) or other means.

4D continues iterating processes while any process is still alive, for a maximum duration of 13 seconds. To respond to a quit action, we recommend to set a flag (such as an entry in the [Storage](storage) object) in the On Exit database method or in your custom quit method. It could make sense to loop in that method (max 10 seconds) to give time to your running processes to respond and quit themselves properly. It is not recommended to wait longer than 10 seconds, to avoid that 4D kills the process or the operating system kills the whole application.
