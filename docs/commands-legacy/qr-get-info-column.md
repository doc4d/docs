...
**Note:** The command does not take into account any virtual structure defined by means of the [SET TABLE TITLES](set-table-titles) and [SET FIELD TITLES](set-field-titles) commands. The actual name of the field is returned in the *object* parameter. 
...
You will notice that since you want to alter only the column size, you have to use QR GET INFO COLUMN to retrieve the column properties and pass them to [QR SET INFO COLUMN](qr-set-info-column) to leave it unchanged, except for the column size.
