...
The command returns the current resizing options, as set in the Design mode or for the process using the [OBJECT SET RESIZING OPTIONS](object-set-resizing-options) command. These options specify the display of the object when the form window is resized.
