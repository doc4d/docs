...
<!--REF #_command_.WP Get style sheet.Summary-->The **WP Get style sheet** command returns the style sheet object designated by the *styleSheetName* or by the style sheet name and the *listLevelIndex* in the case of a [hierarchical paragraph style sheet](../user-legacy/stylesheets.md#hierarchical-paragraph-style-sheets).<!-- END REF-->
...
If the style sheet is part of a hierarchical paragraph style sheet, you can optionally specify the *listLevelIndex* parameter to retrieve a specific level of the hierarchy.
...
* If the style sheet is not a list style sheet and `listLevelIndex` is greater than 1, a null object is returned.
...
To retrieve a specific level of a hierarchical paragraph style sheet:
...
$rootStyle:=WP Get style sheet(wpArea;"MainList";1)
