...
O evento `On After Keystroke` não é gerado:
...
When an entry requires a sequence of keystrokes, the [`On Before Keystroke`](onBeforeKeystroke.md) and [`On After Keystroke event`] events are generated only when the entry is fully validaded by the user. O comando `Keystroke` retorna o caractere validado. Este caso ocorre principalmente:
