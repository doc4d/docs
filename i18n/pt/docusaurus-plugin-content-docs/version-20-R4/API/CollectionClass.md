...
| Resultados | Collection                                                              |  <- | Nova coleção                               | <!-- END REF --> |
...
| Resultados   | Real, Undefined |  <- | Arithmetic mean (average) of collection values | <!-- END REF --> |
...
| Parâmetro  | Tipo       |     | Descrição                                     |                  |
| ---------- | ---------- | :-: | --------------------------------------------- | ---------------- |
| Resultados | Collection |  <- | Original collection with all elements removed | <!-- END REF --> |
...
| Resultados | Collection                                                     |  <- | New collection with value(s) added to the original collection                               | <!-- END REF --> |
...
| Resultados   | Collection |  <- | Deep copy of the original collection                                                                | <!-- END REF --> |
...
| ------------ | ---- | :-: | ---------------------------------------------------- | ---------------- |
| propertyPath | Text |  -> | Rota de propriedade objeto a ser usado para cálculos |                  |
| Resultados   | Real |  <- | Number of elements in the collection                 | <!-- END REF --> |
...
| Resultados   | Collection |  <- | New collection with only distinct values  | <!-- END REF --> |
...
| Resultados  | Parâmetros |  <- | True if collections are identical, false otherwise                                  | <!-- END REF --> |
...
| Resultados | Parâmetros   |  <- | True if all elements successfully passed the test                     | <!-- END REF --> |
...
| startFrom  | Integer    |  -> | Índice onde inicia a pesquisa                 |                  |
| Resultados | Parâmetros |  <- | True if _toSearch_ is found in the collection | <!-- END REF --> |
...
| Parâmetro  | Tipo |     | Descrição                  |                  |
| ---------- | ---- | :-: | -------------------------- | ---------------- |
| Resultados | any  |  <- | Last element of collection | <!-- END REF --> |
...
| methodName | Text         |  -> | Nome da função a qual se chama para processar os elementos da coleção |                  |
| param      | any          |  -> | Parameter(s) to pass to _formula_ or _methodName_  |                  |
| Resultados | Collection   |  <- | Collection of transformed values                                      | <!-- END REF --> |
...
| Resultados   | Boolean, Text, Number, Collection, Object, Date |  <- | Maximum value in the collection                       | <!-- END REF --> |
...
| ------------ | ----------------------------------------------- | :-: | ----------------------------------------------------- | ---------------- |
| propertyPath | Text                                            |  -> | Rota de propriedade objeto a ser usado para avaliação |                  |
| Resultados   | Boolean, Text, Number, Collection, Object, Date |  <- | Minimum value in the collection                       | <!-- END REF --> |
...
Se não passar nenhum parâmetro, a função ordena os valores escalares da coleção em ordem ascendente (outros tipos de elementos, como objetos ou coleções, se devolvem desordenados). You can modify this automatic order by passing the `ck ascending` or `ck descending` constants in the _ascOrDesc_ parameter (see below).
...
| Resultados | Collection   |  <- | Sorted copy of the collection (shallow copy)       | <!-- END REF --> |
...
| Resultados | Collection |  <- | Original collection containing added elements | <!-- END REF --> |
...
| Resultados    | Collection |  <- | Element(s) matching queryString in the collection                 | <!-- END REF --> |
...
| Resultados | Text, Number, Object, Collection, Date, Boolean |  <- | Result of the accumulator value                                                   | <!-- END REF --> |
...
| Resultados | Parâmetros   |  <- | True if at least one element successfully passed the test             | <!-- END REF --> |
