...
| ---------- | ----------------------------- | -- | -------------------------------------------- |
| localID    | Text                          | -> | ID local do datastore remoto a ser retornado |
...
O comando `Open datastore` <!-- REF #_command_.Open datastore.Summary -->conecta o aplicativo ao banco de dados 4D identificado pelo parâmetro *connectionInfo*<!-- END REF --> e retorna um objeto `cs.DataStore` correspondente associado ao alias local *localID*.

O banco de dados *connectionInfo* 4D deve estar disponível como um armazenamento de dados remoto, ou seja:

- seu servidor web deve ser lançado com http ou https ativado,
- O armazenamento de dados deve ser exposto ([**Expor como servidor REST**](REST/configuration.md#starting-the-rest-server) opção marcada), bem como [dataclasses e atributos](../REST/configuration.md#exposing-tables-and-fields).

:::note

As solicitações `Open datastore` dependem da API 4D REST e podem exigir uma licença 4D Client para abrir a conexão. Consulte a seção [user login mode](../REST/authUsers.md#user-login-modes) para saber como configurar a autenticação dependendo do modo de login do usuário atual selecionado.

:::

Se não for encontrado um banco de dados correspondente, `Open datastore` retornará **Null**.
...
Passe em connectionInfo um objeto que desceva o armazém de dados remoto ao que quiser se conectar. Todas as propriedades são opcionais\* excepto *hostname* :
...
| passwordAlgorithm | Text       | Passe "4d-rest-digest" se o servidor validar a senha usando o comando [`Validate password`] (https://doc.4d.com/4dv20/help/command/en/page638.html) com o parâmetro *digest* definido como `true`.                                                                                           |
...
Cada classe de dados em um datastore está disponível como uma propriedade do [objeto DataStore] (ORDA/dsMapping.md#datastore)data. O objeto retornado <!-- REF DataStoreClass.dataclassName.Summary -->contém uma descrição da dataclass<!-- END REF -->.
...
// Obtém todos os contextos remotos (neste caso, contextA e contextB)
...
> - A função `.provideDataKey()` deve ser chamada em um banco de dados criptografado. Se for chamado em um banco de dados não criptografado, o erro 2003 (a chave de criptografia não corresponde aos dados). retornado Use o comando `Data file encryption status` para determinar se o banco de dados está criptografado.
...
Por padrão, quando a função não é chamada, o acesso aos dados é sempre concedido na porta de administração web para uma sessão com privilégio `WebAdmin` utilizando o Explorador de Dados. Em algumas configurações, por exemplo, quando o servidor de aplicações estiver alojado em uma máquina de terceiros, é possivel que não quiser que o administrador possaa ver seus dados, mesmo que possa editar a configuração do servidor, incluindo a configuração da <a href="Admin/webAdmin.md#access-key">access key</a>.
