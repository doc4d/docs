...
Cuando ambas funciones están definidas, la propiedad calculada es **read-write**. Si solo se define una `Function get`, la propiedad calculada es **de solo lectura**. Neste caso, é devolvido um erro se o código tentar modificar a propriedade. If only a `Function set` is defined, 4D returns _undefined_ when the property is read.</p><p spaces-before="0" line-breaks-before="2">The type of the computed property is defined by the `$return` type declaration of the *getter*.</p><p spaces-before="
