...
<!--REF #_command_.GRAPH SETTINGS.Summary-->GRAPH SETTINGS permite modificar os parâmetros dos gráficos mostrados em um formulário.<!-- END REF--> O gráfico deve ter sido definido com o comando [GRAPH](graph). GRAPH SETTINGS não tem efeito em um gráfico de tipo setores. Este comando deve ser chamado obrigatoriamente no mesmo processo que o formulário.

**Nota:** não deve chamar este comando se o gráfico foi gerado utilizando o comando [GRAPH](graph) com o parámetro *grafParams* de tipo [Objeto](# "Estruturas de dados como objetos 4D nativos"). Consulte a descrição do comando [GRAPH](graph) para mais informação.
