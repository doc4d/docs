...
Nota: se desejar personalizar o ícone de cada elemento na lista, utilize o comando [SET LIST ITEM PROPERTIES](set-list-item-properties).  
...
**Nota:** SET LIST PROPERTIES  afeta a aparência dos nós na lista hierárquica. Se preferir personalizar o ícone de cada elemento na lista, utilize o comando .[SET LIST ITEM PROPERTIES](set-list-item-properties). 
...
\- por programação, utilizando os comandos [SELECT LIST ITEMS BY POSITION](select-list-items-by-position) e [SELECT LIST ITEMS BY REFERENCE](select-list-items-by-reference).  
