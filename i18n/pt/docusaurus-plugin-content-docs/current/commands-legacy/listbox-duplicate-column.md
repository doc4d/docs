...
* List boxes do tipo Coleção ou Seleção de Entidades passe um ponteiro Nil (->\[\]) em varCol; deverá chamar a [LISTBOX SET COLUMN FORMULA](listbox-set-column-formula) para especificar a expressão fonte de dados.
...
Note que as variáveis de cabeçalho e de rodapé de página sempre se criam com um tipo específico (inteiro longo e texto, respectivamente). Pelo contrário, as variáveis de coluna não podem ser escritas durante a criação porque os list boxes aceitam diferentes tipos de arrays para estas variáveis (array texto, array inteiro, etc.). Isto significa que você têm que fixar o tipo de array manualmente (ver exemplo 2). É importante digitar antes de chamar a comandos como [LISTBOX INSERT ROWS](listbox-insert-rows) para inserir novos elementos no array. Ou bem, pode utilizar [APPEND TO ARRAY](append-to-array) tanto para definir o tipo do array e a inserção de elementos.
