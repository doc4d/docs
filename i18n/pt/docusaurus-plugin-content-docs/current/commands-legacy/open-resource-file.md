...
Lembre chamar [CLOSE RESOURCE FILE](close-resource-file) para o arquivo de recursos. Note, entretanto, que 4D fecha automaticamente todos os arquivos de recursos abertos utilizando **Open resource file**.  
  
Ao contrário do comando [Open document](open-document), que abre por padrão um documento com um acesso exclusivo em leitura escritura, **Open resource file**, permite abrir um arquivo de recursos já aberto desde a sessão 4D. Por exemplo, se tentar abrir o mesmo documento duas vezes com [Open document](open-document), um erro de E/S será devolvido na segunda tentativa. Por outro lado, se tentar abrir um arquivo de recursos já aberto desde a sessão 4D, **Open resource file** devolverá seu número de referência. Mesmo se abrir um arquivo de recursos várias vezes, necessita chamar [CLOSE RESOURCE FILE](close-resource-file) uma única vez para fechar esse arquivo. Note que este funcionamento é válido apenas se o arquivo de recursos estiver aberto desde a sessão 4D; Se tentar abrir um arquivo de recursos já aberto por outra aplicação, obterá um erro E/S.
