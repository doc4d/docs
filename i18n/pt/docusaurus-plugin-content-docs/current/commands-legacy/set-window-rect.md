...
**Aviso**: utilize este comando com precaução, poderia mover uma janela mais além dos limites da janela principal (em Windows) ou da tela (em Macintosh). Para evitar isto, utilize comandos como [Screen width](screen-width) e [Screen height](screen-height) para verificar as novas coordenadas da janela.  
