...
Se um nome padrão é associado com o campo imagem, ele é fornecido na caixa de diálogo (ver o comando [SET PICTURE FILE NAME](set-picture-file-name)).

Passe em *imagem* a variável ou campo imagem que contém a imagem a armazenar no disco.

O parâmetro opcional *codec* permite definir o formato no qual a imagem será guardado. Um codec pode ser uma extensão (por exemplo ".gif") ou um tipo Mime (por exemplo "image/jpg") Pode obter uma lista de codecs disponíveis através do comando [PICTURE CODEC LIST](picture-codec-list).
