...
**Note**: Se passar True ao comando [SET AUTOMATIC RELATIONS](set-automatic-relations) durante a mesma sessão, as chamadas ao comando SET FIELD RELATION se ignoram, sem importar se estão localizadas antes ou depois de [SET AUTOMATIC RELATIONS](set-automatic-relations). Para "bloquear" o modo automático e ter em conta chamadas a SET FIELD RELATION, passe False a [SET AUTOMATIC RELATIONS](set-automatic-relations).
