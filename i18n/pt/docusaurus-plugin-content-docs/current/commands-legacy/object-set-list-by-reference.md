...
Em *lista*, passe o número de referência da lista hierárquica que deseja associar ao objeto. Esta lista deve ter sido gerada utilizando o comando [Copy list](copy-list), [Load list](load-list) ou [New list](new-list).  
  
Para finalizar a associação de uma *lista* com um *objeto*, só passe 0 no parâmetro *lista* para o tipo de lista relativa. Eliminar uma associação de lista, não elimina a referência de lista em memoria. Lembre de chamar o comando [CLEAR LIST](clear-list) quando já não necessite a lista.
