...
**Nota**: O comando não inclui partes list box. Para passar um formulário para objetos list box, deve usar [LISTBOX GET OBJECTS](listbox-get-objects) ou [LISTBOX GET ARRAYS](listbox-get-arrays) (ver exemplo abaixo para uma combinação de [FORM GET OBJECTS](form-get-objects), [OBJECT Get type](object-get-type) e [LISTBOX GET OBJECTS](listbox-get-objects)).
...
Passe o nome do array ponteiro que já contém os ponteiros às variáveis ou campos associados aos objetos no parâmetro opcional variablesArray. Se um objeto não tiver uma variável associada, o ponteiro [Is nil pointer](is-nil-pointer) é retornado. Se houver um objeto de tipo “subformulário”, é retornado um ponteiro à tabela do subformulário.
