...
* uma chamada ao comando [SET APPLICATION COLOR SCHEME](set-application-color-scheme);
* Se [SET APPLICATION COLOR SCHEME](set-application-color-scheme) não for chamado ou então for chamado com o parâmetro "inherited/herdado", vá em Configurações (Configurações do banco de dados host no caso de um componente);
...
Veja a descrição do comando [SET APPLICATION COLOR SCHEME](set-application-color-scheme) para detalhes sobre os nomes de esquema de cores. 
