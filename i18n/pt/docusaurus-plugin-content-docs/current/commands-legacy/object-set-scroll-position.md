...
<!--REF #_command_.OBJECT SET SCROLL POSITION.Summary-->O comando OBJECT SET SCROLL POSITION permite deslocar o conteúdo de vários tipos de objetos: linhas de um subformulário, de um formulário listado mostrado utilizando os comandos [MODIFY SELECTION](modify-selection), [DISPLAY SELECTION](display-selection) ou de uma lista hierárquica, linhas e colunas de um list box ou inclusive os píxels de uma imagem.<!-- END REF-->  
...
**Nota:** O comando [HIGHLIGHT RECORDS](highlight-records) tem um parâmetro opcional *\** que permite delegar a gestão de rolagem nos formulários ao comando OBJECT SET SCROLL POSITION . 
