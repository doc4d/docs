...
* *dbContextInfo* (objeto): informação relativa as operações manipuladas pelo motor da base de dados. Contém as seguintes propriedades:  
   ...
   * *client\_version* (cadeia): quatro dígitos representam a versão do motor 4D da aplicação, como os devolveu o comando [Application version](application-version).  
