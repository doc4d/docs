...
<!--REF #_command_.JSON Parse.Summary-->O comando **JSON Parse** analisa o conteúdo de uma cadeia com formato JSON e extraem os valores que podem armazenar em um campo ou variável 4D.<!-- END REF--> Este comando não realiza os dados JSON, realiza a ação inversa do comando [JSON Stringify](json-stringify).  
...
**Nota**: Se utilizar ponteiros, deve chamar ao comando [JSON Stringify](json-stringify) antes de chamar a **JSON Parse**.  
...
Este exemplo mostra o uso combinado dos comandos [JSON Stringify](json-stringify) e **JSON Parse**:
