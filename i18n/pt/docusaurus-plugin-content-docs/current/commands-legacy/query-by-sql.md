...
O comando QUERY BY SQL é similar ao comando [QUERY BY FORMULA](query-by-formula). O comando procura registros na tabela especificada. Modifica a seleção atual de *tabela* para o processo atual e transforma o primeiro registro da nova seleção no registro atual.  
...
**Nota:** este comando é compatível com os comandos [SET QUERY LIMIT](set-query-limit) e [SET QUERY DESTINATION](set-query-destination)  
...
Utilizando o comando [QUERY BY FORMULA](query-by-formula), pode escrever:  
...
**Nota**: QUERY BY SQL trata as relações Um-a-Muitos e Muitos-a-Um de uma maneira diferente que [QUERY BY FORMULA](query-by-formula).
