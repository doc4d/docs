...
<!--REF #_command_.CREATE RECORD.Summary-->CREATE RECORD cria um novo registro vazio para *tabela*, mas não mostra o novo registro.<!-- END REF--> Utilize o comando [ADD RECORD](add-record) para criar um novo registro e mostra-lo em um formulário de entrada.

CREATE RECORD é utilizado ao invés de [ADD RECORD](add-record) quando os valores dos registros são introduzidos por programação. O novo registro é convertido no registro atual para a seleção atual mas a seleção atual não se modifica.  
  
O registro existe em memória unicamente até que um comando [SAVE RECORD](save-record) é executado para a tabela. Se muda o registro atual (por exemplo, por uma pesquisa) antes de guardar o registro, o novo registro se perde.
