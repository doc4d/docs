...
* A estrutura virtual definida pelos comandos [SET TABLE TITLES](set-table-titles) e [SET FIELD TITLES](set-field-titles) (se houver) não pode ser usada na variável fórmula - nem será usada por 4D na variável retornada. A estrutura virtual é usada apenas na caixa de diálogo editor Fórmula. 

Lembre que quando se valida a caixa de diálogo, o comando não executa a *formula*; só valida e atualiza o conteúdo da variável. Se quer executar a *formula*, deve utilizar o comando [EXECUTE FORMULA](execute-formula) . 
