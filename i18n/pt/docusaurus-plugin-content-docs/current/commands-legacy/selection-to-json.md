...
**Nota:** Depois de uma chamada a **Selection to JSON**, a seleção atual e registro atual permanecem o mesmo, mas o registro atual não é mais carregado. e poderia ter mudado (o último registro da seleçao atual é então o registro atual). Depois do comando Selection to JSON, utilize os comandos [LOAD RECORD](load-record) em combinação com [GOTO SELECTED RECORD](goto-selected-record) (se for necessário) utilize os valores dos campos no registro atual.
