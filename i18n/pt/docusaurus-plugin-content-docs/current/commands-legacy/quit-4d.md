...
* Se houver um método [Método banco de dados On Exit](metodo-banco-de-dados-on-exit), 4D começa a executar esse método dentro de um processo local recém-criado. Por exemplo, você pode usar esse método de banco de dados para informar os outros processos, por meio da comunicação entre processos, que eles devem fechar (entrada de dados) ou interromper a execução das operações iniciadas no método [Método banco de dados On Startup](metodo-banco-de-dados-on-startup) (conexão de 4D para outro servidor de banco de dados). Note que 4D poderá eventualmente fechar, o método [Método banco de dados On Exit](metodo-banco-de-dados-on-exit) pode realizar todas as operações de limpeza ou o fechamento quiser, mas não pode recusar a sair e irá fazê-lo, em algum ponto final.
* Se não houver nenhum método [Método banco de dados On Exit](metodo-banco-de-dados-on-exit), 4D anula processo em execução um por um, sem distinção.
...
* Lidar com essas operações a partir do banco no método [Método banco de dados On Exit](metodo-banco-de-dados-on-exit).
...
[Método banco de dados On Exit](metodo-banco-de-dados-on-exit)  
