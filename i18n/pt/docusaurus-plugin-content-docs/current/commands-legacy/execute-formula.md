...
* Se l *instrução* for um método projeto, se recomenda utilizar [EXECUTE METHOD](execute-method) que lhe permite passar parâmetros.
...
O tamanho da cache é zero como padrão (sem caché); deve ser criada ou ajustada com o comando [SET DATABASE PARAMETER](set-database-parameter). Por exemplo:
