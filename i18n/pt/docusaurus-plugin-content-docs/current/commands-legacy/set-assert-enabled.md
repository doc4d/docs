...
<!--REF #_command_.SET ASSERT ENABLED.Summary-->O comando SET ASSERT ENABLED é utilizado para desativar ou reativar as asserções inseridas no código 4D da aplicação.<!-- END REF--> Para maior informação sobre asserções, consulte a descrição do comando [ASSERT](assert). 
...
Por favor considere que quando as asserções estão desativadas, as expressões passadas aos comandos [ASSERT](assert) não são avaliadas. As linhas de código que chamam a [ASSERT](assert) não têm mais efeito no funcionamento da aplicação, nem en termos de comportamento nem em termos de rendimento. 
