...
Se os parámetros são válidos, a atualização fica "em espera" na sessão até que se chame o comando [RESTART 4D](restart-4d). Foi executada várias vezes **SET UPDATE FOLDER** antes de chamar a \[[RESTART 4D](restart-4d), se leva em conta a última chamada válida.  
...
Exceção: Se a ferramenta "updater" não pode criar o arquivo de histórico, se mostra uma caixa de diálogo de alerta, independentemente do valor do parâmetro *errosDiscretos*. Para mais informação, consulte a descrição do comando [Get last update log path](last-update-log-path).  
