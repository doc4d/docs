...
Quando for executado o comando, o *alias* é usado onde for devolvido ou mostrado a conta de usuário 4D atual, em particular nos comandos [Current user](current-user), [Process activity](../commands/process-activity) e [LOCKED BY](locked-by), ou o [ORDER BY FORMULA](order-by-formula) da janela de administração de 4D Server. Em ambientes cliente/servidor, este comando deve ser executado em aplicações remotas para que cada usuário se identifique no servidor, por exemplo, mediante o comando [Current user](current-user), mesmo se todas as aplicações remotas utilizam a mesma conta de usuário 4D.

Para eliminar o *alias* de usuário 4D atual, chame a **SET USER ALIAS**("").

**Nota**: ao chamar ao comando [CHANGE CURRENT USER](change-current-user) se restabelece o nome de alias para o usuário atual.
