...
Se Locked devolve TRUE, o registro não pode ser guardado porque está bloqueado por outro usuário ou processo, ou esta empilhado no processo atual. Neste caso, utilize [LOAD RECORD](load-record)  para recarregar o registro até que Locked devolva FALSE.  
...
Se carrega um registro que tenha sido apagado, Locked devolve TRUE. Para evitar esperar por um registro que não existe, utilize o comando [LOCKED BY](locked-by). Se o registro tiver sido apagado, o comando [LOCKED BY](locked-by) devolve -1 no parâmetro processo.  
...
Durante uma transação, [LOAD RECORD](load-record) e Locked se utilizam com frequência para provar a disponibilidade dos registros. Se um registro estiver bloqueado, é comum cancelar a transação.
