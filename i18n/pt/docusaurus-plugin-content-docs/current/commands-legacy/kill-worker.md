...
Se **KILL WORKER** for aplicado a um worker que não foi criado explícitamente utilizando o comando [CALL WORKER](call-worker) (por exemplo, o proceso worker principal da aplicação), só pede a este trabalhador esvaziar sua caixa de mensagens. Em consequência, **KILL WORKER* (1)* não faz nada.

Se chamar ao comando [CALL WORKER](call-worker) para enviar uma mensagem a um worker que acaba de ser finalizado por **KILL WORKER**, um novo processo é iniciado. Para ter certeza de que só há um processo que se executa por vez para um trabalhador, o novo processo se iniciará assim que o anterior tenha finalizado. Entrentato, lembre que se [CALL WORKER](call-worker) for chamado por um worker para ser enviada uma mensagem, enquanto acaba de ser eliminado por **KILL WORKER**, o comando não faz nada.
