...
RECEIVE BUFFER é diferente de [RECEIVE PACKET](receive-packet), ele leva o que está no buffer e, em seguida, retorna imediatamente. [RECEIVE PACKET](receive-packet) aguarda até encontrar um caractere específico ou até que um determinado número de caracteres estejam no buffer
...
Note que o acesso à variável interprocesso *vtBuffer* deve ser protegida por um semáforo, de modo que os processos não causarão conflito. Veja o comando [Semaphore](semaphore) para obter mais informações.
