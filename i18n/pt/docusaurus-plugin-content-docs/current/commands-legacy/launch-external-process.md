...
<!--REF #_command_.LAUNCH EXTERNAL PROCESS.Summary-->A partir de 4D v19 R4, recomendamos o uso de [`4D.SystemWorker class`](../API/SystemWorkerClass) para rodar e controlar processos externos.<!-- END REF--> Entretanto, esse comando ainda é compatível 
...
4D oferece três variáveis de ambiente específicas que podem ser configuradas utilizando [SET ENVIRONMENT VARIABLE](set-environment-variable) e estão disponíveis para seu uso no contexto de **LAUNCH EXTERNAL PROCESS**:
...
Se definir a variável do ambiente *\_4D\_OPTION\_BLOCKING\_EXTERNAL\_PROCESS* como "false" via o comando [SET ENVIRONMENT VARIABLE](set-environment-variable) (execução assincrônica), os parâmetros *fluxoSaida* e *fluxoErro* não são devolvidos.
