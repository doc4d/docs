...
* Evitar a abertura da base se não se cumpre uma condição (ausência de recursos sistema) para uma chamada explícita a [QUIT 4D](quit-4d).
* Realizar outras ações que queira fazer automaticamente cada vez que seja aberta a base.

Para executar código automaticamente na máquina cliente quando um 4D remoto se conecta ao servidor, utilize o [Método banco de dados On Startup](metodo-banco-de-dados-on-startup).
