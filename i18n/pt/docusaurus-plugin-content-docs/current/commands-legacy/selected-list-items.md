...
**Nota:** em caso de seleção múltipla, o comando retorna a posição ou referência do elemento atual de *lista*. O elemento atual é o último item no que o usuário clica (seleção manual) ou o elemento designado pelos comandos #cmd id="381"/\] ou [SELECT LIST ITEMS BY REFERENCE](select-list-items-by-reference) (seleção por programação). 
