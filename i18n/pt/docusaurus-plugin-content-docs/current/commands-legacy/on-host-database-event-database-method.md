...
| On before host database exit    | Inteiro longo | 3     | A base local está fechando. O [Semaphore](../commands/semaphore) da base local ainda não foi chamado. <br/>O [Semaphore](../commands/semaphore) da base local não é chamado enquanto o [On Host Database Event database method](on-host-database-event-database-method.md) do recipiente esteja executando |
