...
**Nota:** Quando você passa um BLOB no *pacote*, o comando não leva em conta qualquer conjunto de caracteres definido pelo comando [USE CHARACTER SET](use-character-set). O BLOB é enviado sem nenhuma modificação.

Antes de usar SEND PACKET, você deve abrir uma porta serial ou um documento com [SET CHANNEL](set-channel), ou abrir um documento com um dos comandos de documento.

Ao escrever um documento, o primeiro SEND PACKET começa a escrever no início do documento a menos que o documento seja aberto com [USE CHARACTER SET](use-character-set). Até que o documento seja fechado, cada pacote subseqüente é adicionado a quaisquer pacotes enviados anteriormente.

**Nota:** Este comando é útil para um documento aberto com [SET CHANNEL](set-channel). Por outro lado, para um documento aberto com o [Open document](open-document), [Create document](create-document) ou [Append document](append-document), você pode usar os comandos [Get document position](get-document-position) e [SET DOCUMENT POSITION](set-document-position) para obter e alterar o local no documento onde a próxima escrita (SEND PACKET) ou leitura ([RECEIVE PACKET](receive-packet)) irá ocorrer.
