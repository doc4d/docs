...
<!--REF #_command_.BLOB to print settings.Summary-->O comando **BLOB to print settings** substitui a configuração de impressão 4D atual pelos parâmetros armazenados no BLOB *confImpr*.<!-- END REF--> Este BLOB deve ter sido gerado pelo comando [Print settings to BLOB](print-settings-to-blob) ou pelo comando 4D Pack  4D Pack (ver abaixo).
...
Os ajustes de impressão são aplicadas [ às configurações de impressão 4D](../settings/compatibility), sempre e cuando nenhum comando como *\_o\_PAGE SETUP*, [SET PRINT OPTION](set-print-option) ou [PRINT SELECTION](print-selection) sem o parâmetro > os modifique. O conjunto de parâmetros se utiliza particularmente para os comandos [PRINT SELECTION](print-selection), [PRINT LABEL](print-label), [PRINT RECORD](print-record), [Print form](../commands/print-form) e [QR REPORT](qr-report), assim como para os comandos de menu de 4D, incluindo as do ambiente Desenho.

Os comandos [PRINT SELECTION](print-selection), [PRINT LABEL](print-label) e [PRINT RECORD](print-record) devem ser chamados com o parâmetro *\>* (se aplicado) para que os parâmetros definidos por **BLOB to print settings** se mantenham.
...
Os ajustes de impressão BLOB gerados pelo comando  legacy 4D podem ser carregados e utilizados pelo comando **BLOB to print settings**. Lembre que se são salvos por [Print settings to BLOB](print-settings-to-blob), são convertidos e não serão abertos mais por . O comando**BLOB to print settings** armazena mais informação sobre a impressão que . 
