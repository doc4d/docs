...
<!--REF #_command_.BREAK LEVEL.Summary-->BREAK LEVEL especifica o número de níveis de quebra em um relatório realizado utilizando [PRINT SELECTION](print-selection).<!-- END REF-->

**Deve** executar BREAK LEVEL e [ACCUMULATE](accumulate) antes de cada relatório no qual queira utilizar quebras. Estes comandos ativam o processo de quebras para um relatório. Ver a explicação para o comando [Subtotal](subtotal).
...
O exemplo a seguir imprime um relatório com dois níveis de quebra. A seleção é ordenada em quatro níveis, mas o comando **BREAK LEVEL** especifica apenas dois níveis de quebra. Um campo se acumula com o comando [ACCUMULATE](accumulate) :
