...
Se *docRef* é especificado, este comando recupera dados de um documento aberto, usando [Open document](open-document), [Create document](create-document) ou [Append document](append-document). Se *docRef* for omitido, o comando recupera dados da porta serial ou o documento aberto usando [SET CHANNEL](set-channel).

Seja qual for a fonte, os caracteres lidos são retornados em *receiveVar*, que deve ser um texto, String ou variável BLOB. Se os caracteres foram enviados pelo comando [SEND PACKET](send-packet), o tipo deve corresponder ao do pacote enviado. 

**Notas:**

* Quando o pacote recebido é do tipo BLOB, o comando não leva em conta qualquer conjunto de caracteres definido pelo comando [USE CHARACTER SET](use-character-set). O BLOB é retornado sem nenhuma modificação
...
* Quando RECEIVE PACKET estiver lendo a partir de uma porta serial, ele vai tentar esperar indefinidamente até que o tempo limite (se houver) haja decorrido (ver [SET TIMEOUT](set-timeout)) ou até que o usuário interrompa a recepção (veja abaixo).
...
**Nota:** Este comando é útil para um documento aberto com [SET CHANNEL](set-channel). Por outro lado, para um documento aberto com o [Open document](open-document), [Create document](create-document) ou [Append document](append-document) você pode usar os comandos [Get document position](get-document-position) e [SET DOCUMENT POSITION](set-document-position) para obter e alterar o local no documento onde acontecerá a próxima escrita ([SEND PACKET](send-packet)) ou leitura (RECEIVE PACKET).
