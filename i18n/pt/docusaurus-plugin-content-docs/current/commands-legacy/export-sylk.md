...
Um termômetro de progresso aparece durante a exportação. O usuário pode cancelar a operação clicando no botão Deter. Se a exportação termina com sucesso, a variável sistema OK toma o valor 1\. Em caso de erro ou se a operação seja interrompida, a variável OK toma o valor 0\. O termômetro pode ser ocultado com o comando [MESSAGES OFF](messages-off). 

O comando utiliza por padrão o conjunto de caracteres UTF-8\. Os documentos em formato SYLK utilizam geralmente o conjunto de caracteres ISO-8859-1, pode utilizar o comando [USE CHARACTER SET](use-character-set) para especificar o conjunto de caracteres apropriado.
