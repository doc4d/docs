...
Exceto pela compatibilidade com arrays (ver a continuação), este comando es idêntico a [SAX OPEN XML ELEMENT](sax-open-xml-element). Por favor consulte a descrição deste comando para maior informação sobre seu funcionamento.  
  
SAX OPEN XML ELEMENT ARRAYS aceita arrays de tipo data, numéricos, booleanos e imagem como parâmetros arrayValoresAtrib. 4D automaticamente realiza todas as conversões necessárias; pode configurar estas conversões utilizando o comando [XML SET OPTIONS](xml-set-options).  
