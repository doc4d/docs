...
Normalmente, a numeração começa em 1\. Pode mudar a numeração para uma tabela utilizando o comando [SET DATABASE PARAMETER](set-database-parameter).  
  
**Nota:** Se não houver um registro atual e a numeração tiver sido modificada via o comando [SET DATABASE PARAMETER](set-database-parameter), este número é em fato reservado para a criação do novo registro mas só vai re ser retornado pela função **Sequence number** quando o comando [SAVE RECORD](save-record) tiver sido chamado. 
