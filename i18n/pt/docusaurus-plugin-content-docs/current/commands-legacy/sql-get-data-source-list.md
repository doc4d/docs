...
4D permite que se conecte diretamente através da linguagem a uma fonte de dados ODBC externa e executar pesquisas SQL dentro de uma estrutura **Begin SQL/End SQ**L. Este princípio funciona desta forma: o comando SQL GET DATA SOURCE LIST permite obter a lista de fontes de dados presentes na máquina. O comando [SQL LOGIN](sql-login) permite determinar a fonte a utilizar. Depois pode executar as pesquisas SQL utilizando uma estrutura **Begin SQL/End SQL** na fonte “atual”. Para realizar novas pesquisas utilizando novamente o motor interno de 4D, simplesmente passe o comando [SQL LOGOUT](sql-logout). Para maior informação sobre os comandos SQL no editor de métodos, consulte *Manual de SQL* no manual 4D SQL.  
