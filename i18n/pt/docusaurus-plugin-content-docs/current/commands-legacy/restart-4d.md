...
Este comando é para uso no contexto de uma aplicação fundida (cliente/servidor ou autônoma) e deve ser utilizado juntamente com o comando [SET UPDATE FOLDER](set-update-folder). Neste caso , o processo de atualização automática é lançado: A nova versão da aplicação designada por [SET UPDATE FOLDER](set-update-folder) substitui automaticamente a versão atual no momento do reinicio resultantes de **RESTART 4D**. A rota de acesso ao arquivo de dados é salva e se utiliza de forma automática.  
  
Se não foi definida informação de atualização utilizando o comando [SET UPDATE FOLDER](set-update-folder) na sessão atual, o comando simplesmente reinicia a aplicação fundida com o mesmo arquivo de dados.  
