...
* Se a área de transferência contém dados de tipo TEXT, pode obter essa informação utilizando o comando [Get text from pasteboard](get-text-from-pasteboard), o qual devolve um valor texto, ou o comando [GET PASTEBOARD DATA](get-pasteboard-data), que devolve o texto em um BLOB.
* Se a área de transferência contém dados de tipo PICT, pode obter esses dados utilizando o comando [GET PICTURE FROM PASTEBOARD](get-picture-from-pasteboard), que devolve a imagem em um campo ou uma variável ou o comando [GET PASTEBOARD DATA](get-pasteboard-data), que devolve a imagem em um BLOB.
* Se o clipboard contém uma rota de acesso ao arquivo, pode extraí-la utilizando o comando [Get file from pasteboard](get-file-from-pasteboard) , que devolve a rota de acesso do arquivo.
* Para qualquer outro tipo de dados, utilize o comando [GET PASTEBOARD DATA](get-pasteboard-data), o qual devolve os dados em um BLOB.
