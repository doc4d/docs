...
<!--REF #_command_.NEXT RECORD.Summary-->NEXT RECORD move o ponteiro do registro atual ao seguinte registro na seleção atual de tabela para o processo atual.<!-- END REF--> Se a seleção atual estiver vazia, ou se [Before selection](before-selection) ou [End selection](end-selection) for TRUE, NEXT RECORD não tem nenhum efeito.  

Se NEXT RECORD move o ponteiro do registro atual ao final da seleção atual, [End selection](end-selection) retorna TRUE, e não houver registro atual. Se [End selection](end-selection) retorna TRUE, utilize [FIRST RECORD](first-record), [LAST RECORD](last-record) ou [GOTO SELECTED RECORD](goto-selected-record) para mover o ponteiro do registro atual de regresso a seleção atual.
