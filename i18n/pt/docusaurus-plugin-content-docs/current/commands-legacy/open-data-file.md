...
Se o caminho de acesso define um arquivo de dados válido, 4D fecha o banco de dados em andamento e re-abre-o com o arquivo de dados especificado. Os métodos de banco de dados [Método banco de dados On Exit](metodo-banco-de-dados-on-exit) e [Método banco de dados On Startup](metodo-banco-de-dados-on-startup) são sucessivamente chamados.

**Atenção:** Uma vez que este comando faz com que a aplicação seja fechada antes de abrir novamente com o arquivo de dados especificado, não é possível usá-lo no método [Método banco de dados On Exit](metodo-banco-de-dados-on-exit) ou em um método chamado por este método de banco de dados.
...
**4D Server:** Começando com 4D v13, este comando pode ser executado com o 4D Server. Neste contexto, o comando faz uma chamada interna ao comando [QUIT 4D](quit-4d) no servidor (o qual faz que apareça uma caixa de diálogo em cada computador remoto indicando que o servidor está no processo de saída) antes de abrir o arquivo designado.
