...
* **Para cifrar o banco de dados pela primeira vez**, só necessita passar o parâmetro *novaFrasePasse* ou *novaChaveDados* (os parâmetros *atualFrasePasse* ou *atualChave* não devem ser proporcionadas):  
   ...
   * *novaChave*: objeto (com propriedade *encodedKey*) que contém uma nova chave de cifrado de dados. Esta chave deveria ter sido gerado com o comando [New data key](new-data-key).  
...
(\*) Advertência devolvida no passo de verificação (consulte o comando [VERIFY DATA FILE](verify-data-file)).
