...
* se for definido um alias (apelido) para o usuário atual com o comando [SET USER ALIAS](set-user-alias), se restabelece mediante a chamada de comando **CHANGE CURRENT USER**..
...
Passe no parâmetro usuário o nome ou o número de referência única (*refUsuário*) da conta a utilizar. Os nomes e os números de usuário podem ser obtidos utilizando o comando [GET USER LIST](get-user-list).  
  
Se a conta de usuário designada não existir ou tiver sido apagada, o erro -9979 é gerado. Pode interceptar este erro com o método de gestão de erros instalado pelo comando [ON ERR CALL](on-err-call.md) . Do contrário, pode chamar a função [Is user deleted](is-user-deleted) para testar a conta de usuário antes de chamar este comando.  
...
\- É possível mostrar a lista de usuários da banco, como na caixa de diálogo de acesso padrão de 4D, utilizando o comando [GET USER LIST](get-user-list)  
...
\- Para que os caracteres de senhas sejam introduzidos de maneira que estejam mascarados em tela, pode utilizar o comando [FILTER KEYSTROKE](filter-keystroke) com a fonte especial *%password*.  
