...
* se "Color Scheme" for estabelecido como "inherited", pela chamada ao comando [SET APPLICATION COLOR SCHEME](set-application-color-scheme);
* se [SET APPLICATION COLOR SCHEME](set-application-color-scheme) não for chamado, ou for chamado com o parâmetro "inherited", use Configurações (Configurações do banco de dados host no caso de um componente);
* se Configurações for estabelecido como "inherited", use as preferências de usuário do sistema operacional

Veja o comando [SET APPLICATION COLOR SCHEME](set-application-color-scheme) para detalhles sobre nomes de esquemas de cores. 
