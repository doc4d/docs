...
<!--REF #_command_.Asserted.Summary-->O comando Asserted tem um funcionamento similar ao do comando [ASSERT](assert), com a diferença de que retorna um valor que é o resultado da avaliação do parâmetro *expressaoBool*.<!-- END REF--> Permite utilizar uma asserção durante a avaliação de uma condição (ver o exemplo). Para maior informação sobre o funcionamento das asserções e os parâmetros deste comando, consulte a descrição do comando [ASSERT](assert).

Asserted aceita uma expressão Booleana como parâmetro e retorna o resultado da avaliação desta expressão. Se a expressão é falsa e se as asserções estão ativas (ver o comando [SET ASSERT ENABLED](set-assert-enabled)), é gerado o erro -10518, exatamente que para o comando [ASSERT](assert). Se as asserções estão inativas, Asserted retorna o resultado da expressão que foi passada sem disparar um erro.

Nota: Como o comando [ASSERT](assert), Asserted funciona em modo interpretado e compilado. 
