...
A unidade utilizada para a *altura* especificada corresponde a definida globalmente para as filas de list box, seja na lista de propriedades ou por uma chamada anterior ao comando [LISTBOX SET ROWS HEIGHT](listbox-set-rows-height).
...
... Então as filas 1, 5 e 11 voltam para a altura global de fila predeterminada estabelecida por [LISTBOX SET ROWS HEIGHT](listbox-set-rows-height) (ou seja, 2 linhas) já que a unidade mudou de píxels a linhas. Já que não há conversão automática aplicada, a mudança de unidades sempre resulta em altura de fila reinicializada no novo valor como padrão definido.
