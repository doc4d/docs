...
•através do comando [SQL EXECUTE](sql-execute),  
...
Para fechar a conexão atual e liberar a memoria, simplesmente execute o comando [SQL LOGOUT](sql-logout). Todas as pesquisas SQL são enviadas então ao banco 4D SQL interno.  
...
Abertura de uma conexão direta com a aplicação 4D Server executada na máquina que tiver a direção IPv6 2a01: e35:2e41:c960:dc39:3eb0:f29b:3747 e responde na porta TCP 20150\. As consultas SQL executadas através de comando [SQL EXECUTE](sql-execute) serão redirigidas a esta conexão; As consultas incluidas nas etiquetas [Begin SQL](begin-sql)/[End SQL](end-sql) não serão redirigidas.
