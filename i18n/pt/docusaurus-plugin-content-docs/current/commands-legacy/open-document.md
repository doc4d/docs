...
Quando um documento for aberto, Open document se localiza inicialmente ao começo do documento, enquanto [Append document](append-document) é localizado ao final do documento.  

Quando abrir um documento, pode ler e escrever no documento utilizando os comandos [RECEIVE PACKET](receive-packet) e [SEND PACKET](send-packet) que pode combinar com os comandos [Get document position](get-document-position) e [SET DOCUMENT POSITION](set-document-position) para acessar diretamente a qualquer parte do documento.

Não esqueça de, no final, chamar [CLOSE DOCUMENT](close-document) para o documento.
