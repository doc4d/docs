...
<!--REF #_command_.NO DEFAULT TABLE.Summary-->O comando NO DEFAULT TABLE permite cancelar o efeito do comando [DEFAULT TABLE](default-table).<!-- END REF--> Depois da execução deste comando, não há tabela padrão definida para o processo.  
Este comando não tenrá efeito se o comando [DEFAULT TABLE](default-table) não tiver sido chamado anteriormente.
