...
- Any text entered in an HTML `comment` tag (*\ ) at the top of the markdown file.</p></li>

- Or, if no html `comment` tag is used, the first sentence after a `# Description` tag of the markdown file.  
  In this case, the first line contains the **prototype** of the method, automatically generated by the 4D code parser.</ul>
...
| `` \`<code>4D <br/>Type your example here \` ``</code> | Used to format 4D code examples (uses highlight.js library)                                                                                          |
