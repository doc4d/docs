...
Normalmente, no es necesario llamar a este comando, ya que 4D guarda las modificaciones de datos regularmente. La opción **Vaciar la caché cada X segundos (minutos)** en [Página Base de datos](../settings/database) de las Propiedades de la base, que especifica con qué frecuencia guardar, se suele utilizar para controlar el vaciado de la caché. Recomendamos utilizar el valor por defecto de 20 segundos. Tenga en cuenta también que el parámetro Cache flush periodicity se puede ajustar y leer utilizando los comandos [SET DATABASE PARAMETER](set-database-parameter) y [Get database parameter](get-database-parameter).
