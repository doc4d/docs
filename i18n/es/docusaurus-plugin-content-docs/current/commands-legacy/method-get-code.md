...
* Los nombres de los comandos se escriben en inglés, excepto si utiliza una versión francesa de 4D y si tiene seleccionada la preferencia "Utilizar lenguaje francés y parámetros regionales sistema" (ver [Is a list](is-a-list)). Cuando se utiliza el parámetro *opcion*, el código puede contener tokens del lenguaje con el fin de que sea independiente del lenguaje de programación 4D y de la versión  (ver más adelante).
...
Pueden definirse atributos adicionales. Para mayor información, consulte la descripción del comando [METHOD SET ATTRIBUTES](method-set-attributes).
...
Consulte el ejemplo del comando [METHOD SET CODE](method-set-code).
