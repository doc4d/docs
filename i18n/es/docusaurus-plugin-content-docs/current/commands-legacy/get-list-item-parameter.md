...
<!--REF #_command_.GET LIST ITEM PARAMETER.Syntax-->**GET LIST ITEM PARAMETER** ( {* ;} *lista* : Integer, Text ; refElem | * ; *selector* : Text ; *valor* : Text, Boolean, Real )<!-- END REF-->
...
Puede pasar un número de referencia en *refElem*. Si este número no corresponde a ningún elemento de la lista, el comando no hace nada. Igualmente puede pasar 0 en *refElem* para indicar el último elemento añadido a la lista (utilizando [APPEND TO LIST](append-to-list)).
...
En *selector*, puede pasar la constante Additional text o Standard action (en el tema “*Listas jerárquicas*”) o todo valor personalizado. Para mayor información sobre los parámetros *selector* y *valor*, consulte la descripción del comando [SET LIST ITEM PARAMETER](set-list-item-parameter).
