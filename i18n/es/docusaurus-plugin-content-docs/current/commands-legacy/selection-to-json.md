...
**Nota:** después de un llamado a **Selection to JSON**, la selección actual no se modifica, pero el registro actual no se carga y podría haber cambiado (el último registro de la selección actual es entonces el registro actual). Después del comando **Selection to JSON**, utilice los comandos [LOAD RECORD](load-record) en combinación con [GOTO SELECTED RECORD](goto-selected-record) (si es necesario) utilice los valores de los campos en el registro actual.
...
[JSON TO SELECTION](json-to-selection)  
