...
La primera sintaxis de **REJECT** evita que el usuario acepte un registro incompleto. Puede alcanzar el mismo resultado sin utilizar **REJECT**, asocie la tecla Intro con un botón Sin acción y utilice los comandos [ACCEPT](accept) y[CANCEL](cancel)  para aceptar o cancelar el registro, una vez los campos hayan sido introducidos correctamente. Es recomendable utilizar esta segunda técnica y no la primera sintaxis de **REJECT**.
...
Puede utilizar [HIGHLIGHT TEXT](highlight-text) para seleccionar los datos en el campo que están siendo rechazados. 
