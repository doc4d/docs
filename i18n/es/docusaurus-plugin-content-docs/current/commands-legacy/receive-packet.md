...
Si se especifica *docRef*, el comando recupera los caracteres desde un documento abierto por la función [Open document](open-document), [Create document](create-document) o [Append document](append-document). Si se omite *docRef*, este comando recupera los caracteres de un puerto serial o de un documento abierto utilizando [SET CHANNEL](set-channel).

Sin importar la fuente, los caracteres leídos se devuelven en la variable *varRecep*, la cual debe ser una variable de tipo Texto, Alfa o BLOB. Si los caracteres han sido enviados por el comando [SEND PACKET](send-packet), el tipo debe corresponder al del paquete enviado. 

**Notas:**

* Cuando el paquete recibido es de tipo BLOB, el comando no tiene en cuenta ningún conjunto de caracteres definido por el comando [USE CHARACTER SET](use-character-set). Se devuelve el BLOB sin ninguna modificación.
...
* Cuando RECEIVE PACKET lee datos de un puerto serial, el comando se ejecutará indefinidamente hasta que el timeout (si hay) haya pasado (ver [SET TIMEOUT](set-timeout)) o hasta que el usuario interrumpa la recepción (ver a continuación).
...
**Nota:** este comando es útil con un documento abierto con [SET CHANNEL](set-channel). Por el contrario, para un documento abierto con [Open document](open-document), [Create document](create-document) o [Append document](append-document) puede utilizar los comandos [Get document position](get-document-position) y [SET DOCUMENT POSITION](set-document-position) para obtener y cambiar la ubicación en el documento donde ocurrirá la próxima escritura ([SEND PACKET](send-packet)) o lectura (RECEIVE PACKET).
