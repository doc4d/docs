...
Este comando debe ser usado en conjunto con el comando [BLOB to print settings](blob-to-print-settings). Estos comandos le permiten guardar la configuración de impresión actual del usuario y recargarla después para que los usuarios no tengan que especificar sus parámetros cada vez que inician un trabajo de impresión. Además, permite mantener la configuración de la impresora "privada" (específica para el controlador de la impresora) no disponible como parámetros de impresión estándar. 

El BLOB generado no debe ser modificado por programación; sólo puede ser utilizado por el comando [BLOB to print settings](blob-to-print-settings).
