...
Una vez haya creado y abierto un documento, puede escribir o leer los valores del documento utilizando los comandos [RECEIVE PACKET](receive-packet) y [SEND PACKET](send-packet) que puede combinar con los comandos [Get document position](get-document-position) y [SET DOCUMENT POSITION](set-document-position) para acceder directamente a ciertas partes del documento.
