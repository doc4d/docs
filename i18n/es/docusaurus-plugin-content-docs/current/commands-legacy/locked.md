...
Si Locked devuelve TRUE, el registro no puede guardarse porque está bloqueado por otro usuario o proceso, o está apilado en el proceso actual. En este caso, utilice [LOAD RECORD](load-record) para recargar el registro hasta que Locked devuelva FALSE.
...
Si trata de cargar un registro que ha sido borrado, Locked devuelve TRUE. Para evitar esperar por un registro que no existe, utilice el comando [LOCKED BY](locked-by). Si el registro ha sido borrado, el comando [LOCKED BY](locked-by) devuelve -1 en el parámetro proceso.
...
Durante una transacción, [LOAD RECORD](load-record) y Locked se utilizan con frecuencia para probar la disponibilidad de los registros. Si un registro está bloqueado, es común cancelar la transacción.
