...
* si guarda una variable de tipo Entero largo que es una referencia a una lista jerárquica (ListRef), VARIABLE TO BLOB guardará la variable Entero largo, no la lista. Para guardar y recuperar las listas jerárquicas en un BLOB, utilice los comandos [LIST TO BLOB](list-to-blob) y [BLOB to list](blob-to-list).
...
Sin embargo, si almacena una variable de tipo [Entero largo](# "Entre -2 millardos y +2 millardos") que es una referencia a una lista jerárquica ([ListRef](# "Una referencia a una lista jerárquica")), VARIABLE TO BLOB almacenará la variable [Entero largo](# "Entre -2 millardos y +2 millardos"), no la lista. Para guardar y recuperar las listas jerárquicas en un BLOB, utilice los comandos [LIST TO BLOB](list-to-blob) y [BLOB to list](blob-to-list).

**Advertencia:** Si utiliza un BLOB para almacenar las variables, debe utilizar posteriormente el comando [BLOB TO VARIABLE](blob-to-variable) para recuperar el contenido del BLOB, porque las variables son almacenadas en BLOBs utilizando un formato interno de 4D.
