...
Puede saber si PRINT SELECTION está imprimiendo el primer encabezado probando [Before selection](before-selection) durante un evento On Header. Igualmente puede verificar el último pie de página, probando [End selection](end-selection) durante un evento On Printing Footer. Para mayor información, consulte la descripción de estos comandos, como también de los comandos [Form event code](../commands/form-event-code) y [Level](level).

Para imprimir una selección ordenada con subtotales o rupturas utilizando PRINT SELECTION, debe primero ordenar la selección. Luego, en cada área de ruptura del informe, incluir una variable con un método de objeto que asigne el subtotal a la variable. Igualmente puede utilizar funciones estadísticas y aritméticas como [Sum](sum) y [Average](average) para asignar valores a las variables. Para mayor información, consulte las descripciones de [Subtotal](subtotal), [BREAK LEVEL](break-level) y [ACCUMULATE](accumulate).

**Advertencia:** no utilice el comando [PAGE BREAK](page-break) con el comando PRINT SELECTION. [PAGE BREAK](page-break) está reservado para ser utilizado con el comando [Print form](../commands/print-form).
