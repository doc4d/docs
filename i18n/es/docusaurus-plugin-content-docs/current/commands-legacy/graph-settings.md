...
<!--REF #_command_.GRAPH SETTINGS.Summary-->GRAPH SETTINGS permite cambiar los parámetros de los gráficos mostrados en un formulario.<!-- END REF--> El gráfico debe haber sido definido con el comando [GRAPH](graph). GRAPH SETTINGS no tiene efecto en un gráfico de tipo sectores. Este comando debe llamarse obligatoriamente en el mismo proceso que el formulario.

**Nota:** no debe llamar este comando si el gráfico se generó utilizando el comando [GRAPH](graph) con el parámetro *grafParams* de tipo [Objeto](# "Datos estructurados como forma de objeto nativo 4D"). Consulte la descripción del comando [GRAPH](graph) para más información.
