...
Cuando se ejecuta el formulario, los mensajes aparecen como mensajes de ayuda cuando el cursor se mueve sobre el campo u objeto. El retardo de visualización y la duración máxima de los mensajes de ayuda se pueden controlar utilizando los selectores Tips delay y Tips duration del comando [SET DATABASE PARAMETER](set-database-parameter).
...
* Los consejos de ayuda pueden desactivarse globalmente para la aplicación utilizando el selector Tips enabled del comando [SET DATABASE PARAMETER](set-database-parameter).
