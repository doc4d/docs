...
<!--REF #_command_.REGISTER CLIENT.Summary-->El comando **REGISTER CLIENT** “registra” un equipo cliente 4D con el nombre especificado en *nomCliente* en 4D Server, con el fin de permitir a otros clientes o posiblemente 4D Server (utilizando procedimientos almacenados) ejecutar métodos utilizando el comando [`EXECUTE ON CLIENT`](execute-on-client).<!-- END REF--> Una vez registrado, un cliente 4D puede ejecutar uno o varios métodos para otros clientes.
...
Cuando se ejecuta este comando, un proceso, llamado *nomClient*, se crea en el equipo cliente. Este proceso sólo puede ser abortado por el comando [`UNREGISTER CLIENT`](unregister-client).   

Una vez ejecutado el comando, no es posible modificar rápidamente el nombre del cliente 4D o el periodo de interrogación al servidor. Para hacerlo, debe llamar al comando [`UNREGISTER CLIENT`](unregister-client), y luego ejecutar el comando **REGISTER CLIENT**.
