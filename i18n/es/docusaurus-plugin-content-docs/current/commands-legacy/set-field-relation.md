...
**Note**: si pasa True al comando [SET AUTOMATIC RELATIONS](set-automatic-relations) durante la misma sesión, las llamadas al comando SET FIELD RELATION se ignoran, sin importar si están ubicadas antes o después de [SET AUTOMATIC RELATIONS](set-automatic-relations). Para "bloquear" el modo automático y tener en cuenta llamadas a SET FIELD RELATION, pase False a [SET AUTOMATIC RELATIONS](set-automatic-relations).
