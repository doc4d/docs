...
* cancelar un registro nuevo o modificado, para el cual la entrada de datos ha sido inicializada utilizando [ADD RECORD](add-record) o [MODIFY RECORD](modify-record).
* cancelar un formulario mostrado por intermedio del comando [DIALOG](../commands/dialog).
* salir de un formulario que muestra una selección de registros, utilizando [DISPLAY SELECTION](display-selection) o [MODIFY SELECTION](modify-selection).
...
**Nota:** esta operación es diferente de la del comando [PAGE BREAK](page-break)(\*) que cancela TODOS los formularios que están en espera de impresión.

### Modo sin interfaz 

El comando **CANCEL** se permite en modo sin interfaz, en el contexto de las áreas fuera de pantalla creadas por *VP Run offscreen area* o [WA Run offscreen area](wa-run-offscreen-area). 
