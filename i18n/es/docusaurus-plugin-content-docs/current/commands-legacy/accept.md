...
* validar un registro o subregistro creado o modificado, para el cual la entrada de datos ha sido inicializada utilizando [ADD RECORD](add-record), [MODIFY RECORD](modify-record), *\_o\_ADD SUBRECORD* o *\_o\_MODIFY SUBRECORD*.
...
También se usa a menudo en el método de caja cierre opcional para el comando [Open window](../commands/open-window). Si hay una caja del menú de control en una ventana, se puede llamar a **ACCEPT** o [CANCEL](cancel), en el método a ejecutar, cuando se hace doble clic en la caja del menú de control o se elige el comando del menú de cierre.
...
El comando **ACCEPT** se permite en modo sin interfaz, en el contexto de las áreas fuera de pantalla creadas por *VP Run offscreen area* o [WA Run offscreen area](wa-run-offscreen-area). 
