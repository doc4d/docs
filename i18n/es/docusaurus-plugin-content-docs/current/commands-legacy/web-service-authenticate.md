...
En los parámetros *nombre* y *contraseña*, pase la información de identificación requerida (nombre de usuario y contraseña). Esta información será codificada y añadida a la petición HTTP enviada al servicio web utilizando el comando [WEB SERVICE CALL](web-service-call). Es entonces necesario llamar al comando **WEB SERVICE AUTHENTICATE** antes de llamar al comando [WEB SERVICE CALL](web-service-call).  
  
La información de autenticación se reinicializa en cero después de cada petición. Por lo tanto, debe utilizar el comando **WEB SERVICE AUTHENTICATE** antes de llamar al comando [WEB SERVICE CALL](web-service-call). 

El parámetro opcional metAutenticacion permite indicar el método de autenticación a utilizar para la próxima llamada del comando [WEB SERVICE CALL](web-service-call). Puede pasar uno de los siguientes valores:
...
Por defecto, la información de autenticación se reinicia después de cada petición. Por lo tanto, debe utilizar el comando **WEB SERVICE AUTHENTICATE** antes de cada comando [WEB SERVICE CALL](web-service-call). Sin embargo es posible conservar temporalmente esta información utilizando una opción del comando [WEB SERVICE SET OPTION](web-service-set-option). En este caso, no es necesario ejecutar el comando **WEB SERVICE AUTHENTICATE** antes de cada [WEB SERVICE CALL](web-service-call).

Si la autenticación falla, el servidor SOAP devuelve un error que puede identificar utilizando el comando [WEB SERVICE Get info](web-service-get-info).
...
[WEB SERVICE Get info](web-service-get-info)  
