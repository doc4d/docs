...
<!--REF #_command_.BLOB to print settings.Summary-->El comando **BLOB to print settings** reemplaza los parámetros de impresión actuales de 4D por los parámetros almacenados en el BLOB *confImpr*.<!-- END REF--> Este BLOB debe haber sido generado por el comando [Print settings to BLOB](print-settings-to-blob).
...
Los parámetros se aplican a los [parámetros actuales de impresión 4D](../settings/compatibility) siempre y cuando ningún comando como [SET PRINT OPTION](set-print-option) o [PRINT SELECTION](print-selection) sin el parámetro > los modifique. Los parámetros definidos se utiliza particularmente para los comandos [PRINT SELECTION](print-selection), [PRINT LABEL](print-label), [PRINT RECORD](print-record), [Print form](../commands/print-form) y [QR REPORT](qr-report), así como también para los comandos de impresión en los menú de 4D, incluyendo los del entorno Diseño.

Los comandos [PRINT SELECTION](print-selection), [PRINT LABEL](print-label) y [PRINT RECORD](print-record) deben ser llamados con el parámetro *\>* (si aplica) para que los parámetros definidos por **BLOB to print settings** se mantengan.
