...
**Nota:** cuando pasa un BLOB en *paquete*, el comando no tiene en cuenta ningún conjunto de caracteres definido por el comando [USE CHARACTER SET](use-character-set). El BLOB se envía sin ninguna modificación.

Antes de utilizar SEND PACKET, debe abrir un puerto serial o un documento con [SET CHANNEL](set-channel), o abrir un documento con uno de los comandos de gestión de documentos.
...
**Nota:** este comando es útil para un documento abierto con [SET CHANNEL](set-channel). Por otra parte, para un documento abierto con [Open document](open-document), [Create document](create-document) o [Append document](append-document), usted puede utilizar los comandos [Get document position](get-document-position) y [SET DOCUMENT POSITION](set-document-position) para obtener y cambiar la ubicación en el documento donde ocurrirá la próxima escritura (SEND PACKET) o lectura ([RECEIVE PACKET](receive-packet)).
