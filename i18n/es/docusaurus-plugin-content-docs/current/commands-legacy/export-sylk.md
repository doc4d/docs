...
<!--REF #_command_.EXPORT SYLK.Syntax-->**EXPORT SYLK** ( {*tabla* : Table ;} *doc* : Text )<!-- END REF-->
...
Un termómetro de progreso aparece durante la exportación. El usuario puede cancelar la operación haciendo clic en el botón Detener. Si la exportación termina con éxito, la variable sistema OK toma el valor 1\. En caso de error o si la operación se interrumpe, la variable OK toma el valor 0\. El termómetro puede ocultarse con el comando [MESSAGES OFF](messages-off). 

El comando utiliza por defecto el conjunto de caracteres UTF-8\. Los documentos en formato SYLK utilizan por lo general el conjunto de caracteres ISO-8859-1, puede utilizar el comando [USE CHARACTER SET](use-character-set) para especificar el conjunto de caracteres apropiado.
