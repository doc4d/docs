...
* un 4D remoto inicia un proceso global, (cuyo nombre o comienza por "$") lo cual necesita de la creación de un proceso cooperativo en el servidor **(\*)**. Este proceso puede crearse utilizando el comando [New process](../commands/new-process.md), un comando de menú o la caja de diálogo "Ejecutar un método".
...
**Importante**: las conexiones web y las conexiones SQL no provocan la ejecución del **Método base On Server Open Connection**. Cuando un navegador web se conecta a 4D Server, se llaman el *Método de base On Web Authentication* (si lo hay) y/o la [Método base On Web Connection](../commands/metodo-base-on-web-connection.md).   
Cuando 4D Server recibe una petición SQL, se llama el [Método base On SQL Authentication](../commands/metodo-base-on-sql-authentication.md)(si existe). Para mayor información, consulte la descripción de este método base en el manual de Lenguaje 4D.
...
Por ejemplo, si un 4D remoto se conecta a una base 4D Server interpretada, se inicia el proceso usuario, el proceso de diseño y el proceso de registro del cliente (por defecto). El Método base On Server Open Connection se ejecuta tres veces seguidas. La primera vez dentro del proceso principal, la segunda vez en el proceso de inscripción del cliente y la tercera vez en el proceso de diseño. Si los tres procesos son respectivamente el sexto, séptimo y octavo proceso a iniciar en el equipo servidor, y si llama [Current process](../commands/current-process.md) desde el Método base On Server Open Connection, la primera vez [Current process](../commands/current-process.md) devuelve 6, la segunda vez 7 y la tercera 8.
...
La información almacenada en esta tabla es administrada por el Método base On Server Open Connection y el [Método base On Server Close Connection](../commands/metodo-base-on-server-close-connection.md) listado a continuación:
