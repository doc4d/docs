...
**Nota**: **Método base On Exit** NO es invocado por 4D Server. Debe utilizar [Método base On Server Shutdown](metodo-base-on-server-shutdown) en este contexto.
...
* Se efectúa una llamada al comando [QUIT 4D](quit-4d).
* Un plug-in 4D hace una llamada al punto de entrada [QUIT 4D](quit-4d).
...
El siguiente ejemplo muestra un caso típico en el que se lanzan uno o más procesos en segundo plano que realizan trabajos regulares, que se ejecutan sin fin, en el [Método base On Startup](metodo-base-on-startup) (o en el [Método base On Server Startup](metodo-base-on-server-startup)). Una bandera en el objeto [Storage](storage) de la aplicación se utiliza para decirles que terminen y para comprobar si han terminado.

* En el [Método base On Startup](metodo-base-on-startup)
...
[Método base On Startup](metodo-base-on-startup)  
[QUIT 4D](quit-4d)  