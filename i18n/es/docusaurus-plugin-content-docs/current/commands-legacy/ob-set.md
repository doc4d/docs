...
* si pasa un puntero, se mantiene como es, se evalúa utilizando el comando [JSON Stringify](json-stringify)
...
Usted quiere exportar datos en JSON que contienen una fecha 4D que desea convertir en una cadena sin información de zona horaria. Note que la conversión ocurre cuando la fecha se guarda en el objeto, debe llamar al comando [SET DATABASE PARAMETER](set-database-parameter) antes de llamar a [OB SET](ob-set): 
