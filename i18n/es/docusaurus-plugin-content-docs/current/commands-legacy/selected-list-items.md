...
**Nota:** en caso de selección múltiple, el comando devuelve la posición o la referencia del primer elemento seleccionado en *lista*, bien sea por un clic del usuario (selección manual) o por los comandos [SELECT LIST ITEMS BY POSITION](select-list-items-by-position) o [SELECT LIST ITEMS BY REFERENCE](select-list-items-by-reference) (selección por programación). 
