...
En *color*, pase un valor de color RGB. Para obtener más información acerca de los colores RGB, consulte la descripción del comando [OBJECT SET RGB COLORS](object-set-rgb-colors). Si desea que la fila herede el conjunto de colores para el nivel más alto, usted puede pasar la constante`lk inherited` en el parámetro *color*.
...
Este comando modifica los valores que se encuentran en el array de colores que pueden haberse definido para la columna del listbox. Si estos arreglos no están ya definidos, el comando crea dinámicamente arrays que puede acceder mediante el comando [LISTBOX Get array](listbox-get-array).  
