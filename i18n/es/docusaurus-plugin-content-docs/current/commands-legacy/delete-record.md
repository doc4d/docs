...
* si el registro actual es descargado de la memoria antes de la llamada a DELETE RECORD (por ejemplo en respuesta a un [UNLOAD RECORD](unload-record)), la selección actual de tabla está vacía después de la eliminación.
* el comando DELETE RECORD no hace nada cuando la tabla está en modo [READ ONLY](read-only), independientemente de si el registro a borrar está bloqueado o no.
