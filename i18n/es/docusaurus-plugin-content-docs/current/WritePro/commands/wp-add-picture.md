...
Para el segundo parámetro opcional, puede pasar cualquiera de los siguientes:

- En *picture*:  una imagen 4D
- En *picturePath*: una cadena que contiene la ruta a un archivo de imagen almacenado en el disco (sintaxis del sistema). Puede pasar una ruta completa o una ruta relativa al archivo de estructura de la base de datos. También puede pasar un nombre de archivo, en cuyo caso el archivo debe estar ubicado junto al archivo de estructura de la base de datos. Si pasa un nombre de archivo, debe indicar la extensión del archivo. 
- En *PictureFileObj*: un objeto `4D.File` que representa un archivo imagen.

:::note

Todo formato imagen [soportado por 4D](../../FormEditor/pictures.md#native-formats-supported) puede ser usado. Puede obtener la lista de formatos de imagen disponibles usando el comando [PICTURE CODEC LIST](../../commands-legacy/picture-codec-list.md). Si la imagen contiene varios formatos (codecs), 4D Write Pro solo conserva un formato para la visualización y otro para la impresión (si son diferentes) en el documento; los "mejores" formatos se seleccionan automáticamente.

:::

- Si se omite *picture*, Se devuelve una referencia de imagen válida y se añade una imagen vacía. Esto le permite llamar a [WP SET ATTRIBUTES](wp-set-attributes.md) con el selector wk image expression para llenar la imagen con una expresión 4D. Si la expresión no se puede evaluar o no devuelve una imagen válida, se muestra una imagen vacía (imagen predeterminada con marco negro).

Por defecto, la imagen añadida es:

- Integrada detrás del texto
- Aparece en la esquina superior izquierda de la papelera
- Mostrar en todas las páginas

La ubicación, la capa (en línea, delante/detrás del texto), la visibilidad y cualquier propiedad de la imagen se pueden modificar utilizando el comando [WP SET ATTRIBUTES](wp-set-attributes.md) o mediante acciones estándar (consulte *Uso de acciones estándar de 4D Write Pro*).

**Nota:** el comando [WP Selection range](../commands-legacy/wp-selection-range.md) devuelve un objeto *referencia a imagen* si se selecciona una imagen anclada y un objeto *rango* si se selecciona una imagen en línea. Puede determinar si un objeto seleccionado es un objeto de imagen comprobando el atributo `wk type`:
...
Desea añadir una imagen con la configuración predeterminada utilizando una ruta de archivo.
...
Desea añadir una imagen redimensionada, centrada y anclada al encabezado:
...
Desea utilizar una expresión de campo para añadir una imagen anclada a un documento que muestra parte del texto de la base de datos:
