...
`.getContent()` 関数は、 <!-- REF #MailAttachmentClass.getContent().Summary -->添付オブジェクトの中身を BLOB として返します<!-- END REF -->。 You can use this method with attachment objects received by the [`MAIL Convert from MIME`](./EmailObjectClass.md#mail-convert-from-mime) command.
