...
| 名称                | データタイプ | とりうる値     |
| ----------------- | ------ | --------- |
| currentItemSource | 文字列    | オブジェクト型の式 |
...
| 名称                        | データタイプ | とりうる値 |
| ------------------------- | ------ | ----- |
| currentItemPositionSource | 文字列    | 数値型の式 |
...
| 名称                 | データタイプ | とりうる値                                              |
| ------------------ | ------ | -------------------------------------------------- |
| dataSourceTypeHint | 文字列    | <li>**リストボックス列:** "boolean", "number", "picture", "text", date", "time"。 *配列/セレクションリストボックスのみ*: "integer", "object"</li><li>**ドロップダウンリスト:** "object", "arrayText", "arrayDate", "arrayTime", "arrayNumber"</li> |
...
| 名称     | データタイプ       | とりうる値                                      |
| ------ | ------------ | ------------------------------------------ |
| values | collection-1 | デフォルト値 (文字列) のコレクション。例: "a", "b", "c", "d" |
...
- **4D式** (単純な式、フォーミュラ、または 4Dメソッド)。 式は値を返す必要があります。 値は `On Display Detail` および `On Data Change` イベントで評価されます。 式の結果は、アプリケーションモードでフォームを実行すると自動で表示されます。 式は、セレクション型リストボックスではマスターテーブルの (カレントまたは命名) セレクションの各レコードごとに、コレクション型リストボックスではコレクションの各要素ごとに、エンティティセレクション型リストボックスではセレクションのエンティティごとに評価されます。 If it is empty, the column will not display any results. The following expression types are supported:
  - String
  - 数値
  - 日付
  - Picture
...
コレクションあるいはエンティティセレクションを使用する場合、カラムに割り当てられた要素プロパティ/エンティティ属性は、通常 [This](https://doc.4d.com/4Dv18/4D/18/This.301-4504875.ja.html) を含む式を用いて宣言します。 この `This` は現在処理中の要素への参照を返す、専用の 4Dコマンドです。 For example, you can use `This.\&#060;propertyPath&#062;` where `\&#060;propertyPath&#062;` is the path of a property in the collection or an entity attribute path to access the current value of each element/entity. If you use a collection of scalar values, 4D will create an object for each collection element with a single property (named "value"), filled with the element value. この場合、`This.value` を式として使用します。
...
| 名称         | データタイプ | とりうる値                     |
| ---------- | ------ | ------------------------- |
| dataSource | 文字列    | 4D変数、フィールド名、あるいは任意のランゲージ式 |
...
| 名称                  | データタイプ | とりうる値   |
| ------------------- | ------ | ------- |
| selectedItemsSource | 文字列    | コレクション式 |
...
| 名称             | データタイプ | とりうる値       |
| -------------- | ------ | ----------- |
| namedSelection | 文字列    | 命名セレクションの名前 |
