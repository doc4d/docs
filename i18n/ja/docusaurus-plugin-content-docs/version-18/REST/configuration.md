...
> - シングルユーザーの4D上では、テスト目的で RESTセッションを 3つ開くことができます。 You need to manage the [session cookie](authUsers.md#session-cookie) to use the same session for your requesting application.
...
2. フィールドの **RESTリソースとして公開** オプションの選択を解除します: ![alt-text](../assets/en/REST/field.png) Repeat this for each field whose exposure needs to be modified.
