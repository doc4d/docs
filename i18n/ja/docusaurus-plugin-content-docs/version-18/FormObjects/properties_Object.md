...
| 名称         | データタイプ            | とりうる値                     |
| ---------- | ----------------- | ------------------------- |
| dataSource | 文字列、または文字列のコレクション | <ul><li>4D変数、フィールド名、あるいは任意のランゲージ式</li><li>Empty string for [dynamic variables](#dynamic-variables).</li><li>String array (collection of array names) for a [hierarchical listbox](listbox_overview.md#hierarchical-list-boxes) column]</li></ul> |
...
| 名称                 | データタイプ | とりうる値                     |
| ------------------ | ------ | ------------------------- |
| dataSourceTypeHint | 文字列    | <ul><li>**standard objects:** "integer", "boolean", "number", "picture", "text", date", "time", "arrayText", "arrayDate", "arrayTime", "arrayNumber", "collection", "object", "undefined"</li><li>**list box columns:** "boolean", "number", "picture", "text", date" (*array/selection list box only*) "integer", "time", "object"</li></ul> |
...
> - footers of [Collection and Entity selection](listbox_overview.md#collection-or-entity-selection-list-boxes) list boxes. これらの場合には、カスタム計算を使用する必要があります。
