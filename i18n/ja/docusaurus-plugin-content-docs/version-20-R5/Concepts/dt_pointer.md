...
- ($PointerTwo->)->:="Goodbye"
  \--> $PointerTwo-> references the contents of $PointerOne, which in turn references $MyVar. つまり、($PointerTwo->)-> は、$MyVar を示しています。 結果として、文字列 "Goodbye" が $MyVar に代入されます。
- ALERT (($PointerTwo->)->)
  \--> Same thing: $PointerTwo-> references the contents of $PointerOne, which in turn references $MyVar. つまり、($PointerTwo->)-> は、$MyVar を示しています。 結果として、アラートボックスには $MyVar の内容が表示されます。
