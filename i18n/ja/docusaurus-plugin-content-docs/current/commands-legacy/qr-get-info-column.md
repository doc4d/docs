...
**注:** このコマンドは、[SET TABLE TITLES](set-table-titles) コマンドと [SET FIELD TITLES](set-field-titles) コマンドによって定義されたバーチャルストラクチャーはどれも考慮に入れません。実際のフィールド名は*object* 引数の中に返されます。
