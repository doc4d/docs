...
既存グループのプロパティを変更する場合は、[GET GROUP LIST](get-group-list) コマンドによって返される有効なグループID番号を渡さなければなりません。
...
グループのすべてのプロパティを変更したいわけではない場合 (メンバーリスト以外、後述参照) 、変更したくないプロパティに関しては、あらかじめ [GET GROUP PROPERTIES](get-group-properties) コマンドを使って取得した値を受け渡します。
