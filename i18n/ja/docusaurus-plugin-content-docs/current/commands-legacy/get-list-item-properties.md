...
項目参照番号を使用して作業を実行する場合、項目がユニークな参照番号を持つリストを作成します。そうでなければ、項目を区別できません。詳細については[APPEND TO LIST](append-to-list)の説明を参照してください。
...
* *icon* には項目に割り当てられたピクチャーが返されます。  
   ...
   **注:** [GET LIST ITEM ICON](get-list-item-icon) コマンドを使用すれば、ピクチャー変数にアイコンを取得できます。
* *color*には項目のテキストカラーが返されます。

これらのプロパティに関する詳細は、[SET LIST ITEM PROPERTIES](set-list-item-properties) コマンドの説明を参照してください。
