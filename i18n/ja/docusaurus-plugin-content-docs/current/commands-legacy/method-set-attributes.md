...
メソッドの属性は[OB SET](ob-set) または [OB SET ARRAY](ob-set-array) コマンドを使用して設定する必要があります。ブール型の属性に対してTrueまたはFalseの値を、拡張された属性に対しては特定の値(例えば4D Mobile Property において"scope":"table" など)を設定します。*attributes* 引数に存在する属性のみがメソッド属性内で更新されます。
