...
**注:** **Modified** は、[PUSH RECORD](push-record) と[POP RECORD](pop-record) コマンド実行後は、常に[True](true.md "True")を返します。

いずれの場合でも、フィールドの値が実際に変更されたかどうかを調べるには、[Old](old) コマンドを使用します。

**注:** **Modified** はあらゆるタイプのフィールドに対して適用できますが、このコマンドを[Old](old) コマンドと組み合わせて使用する場合には、[Old](old) コマンドの制約に注意してください。詳細については[Old](old) コマンドの説明を参照してください。
