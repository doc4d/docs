...
**注:** これは以前の[Self](self)コマンドとまったく同じ動作です。[Self](self)コマンドは互換性の目的でのみ保持されています。
* Object with focus: このセレクタを渡すと、コマンドはフォーム内でフォーカスを持つオブジェクトに割り当てられた変数へのポインタを返します。残り2つのオプション引数は渡されても無視されます。  
**注** **:** これは完全に[Focus object](focus-object)コマンドと同じ動作です。[Focus object](focus-object)コマンドは4D v12で廃止予定となりました。
