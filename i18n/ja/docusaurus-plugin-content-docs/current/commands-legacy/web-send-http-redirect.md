...
このコマンドは同じメソッド内にあるデータを送るコマンド ([WEB SEND FILE](web-send-file)、[WEB SEND BLOB](web-send-blob)など) より優先されます。

また、このコマンドを使用すると、リクエストを他のWebサーバーへリダイレクトすることも可能です。

4DはURLの特殊文字を自動的にエンコードします。 *\** を渡すと、4Dはそれらをエンコードしません。

このコマンドが送信するHTTPステータスコードは**302: Moved Temporarily**です。"301 Moved Permanently"ステータスを送信したい場合は、[WEB SET HTTP HEADER](web-set-http-header)を使用して、"*X-STATUS"フィールドに”301”*を設定します。 
