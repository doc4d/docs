...
このコマンドは [FORM LOAD](../commands/form-load)コマンドを印刷以外の目的で使用したときには必ず呼び出さなければなりません(実際に印刷をしたときには、 [CLOSE PRINTING JOB](close-printing-job) コマンドを呼び出した際に自動的にカレントフォームは再度閉じられます)。
