...
作成するインデックスの名前を*indexName*引数に渡すことができます。インデックスの命名は、複数の異なるタイプのインデックスを同じフィールドに割り当て、それを個々に[DELETE INDEX](delete-index)コマンドで削除する場合に必要となります。*indexName*インデックスが既に存在する場合、コマンドは何もしません。
