...
**Note:** [GET MENU ITEMS](get-menu-items) および [Get menu item](get-menu-item) コマンドはメニュー項目が作られた方法により、メタ文字を返したり返さなかったりします。*\** オプション付きで作成された場合、メタ文字は標準文字として返されます。
...
**重要**: 新しいメニュー項目にはメソッドやアクションが割り当てられていません。これらは[SET MENU ITEM PROPERTY](set-menu-item-property) あるいは [SET MENU ITEM METHOD](set-menu-item-method) コマンドを使用して項目に割り当てるか、[Menu selected](menu-selected) コマンドを使ってフォームメソッドからこれを管理しなくてはなりません。
