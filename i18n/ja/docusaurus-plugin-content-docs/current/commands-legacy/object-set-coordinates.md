...
**注:** このコマンドは、 [OBJECT MOVE](object-move) コマンドに第2引数の *\** を渡した時と同じ動作をします。
...
**注:** オブジェクトのサイズを変更せずに位置だけを変更したい場合は、既存の [OBJECT MOVE](object-move) コマンドの使用が推奨されます。

このコマンドは以下の様な場合においてのみ機能します:

* レコード編集中の入力フォーム
* [DIALOG](../commands/dialog) コマンドを使用して表示したフォーム
* [MODIFY SELECTION](modify-selection) か [DISPLAY SELECTION](display-selection) フォームを使用して表示された出力フォームのヘッダーとフッター
* 印刷中のフォーム
