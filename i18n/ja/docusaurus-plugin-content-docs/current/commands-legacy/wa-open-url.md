...
既存の[OPEN URL](open-url)コマンドのように、**WA OPEN URL**はファイルを指定するための複数のシンタックスを*url* 引数に受け入れます:
...
Mac OSにおいてFileVault が有効化されている場合には、Posix シンタックスを使用する必要があります。[Convert path system to POSIX](convert-path-system-to-posix) コマンドを使用してシステムのパスを変換することができます。
