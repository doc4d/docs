...
**Note:** オブジェクト名に @ 文字を使用することで、フォーム内に名前に対応するオブジェクトが複数検索された場合、[Selected list items](selected-list-items) コマンドは最初に見つけたオブジェクトを処理の対象とします。
...
**Note:** 選択項目が複数存在する場合、コマンドは、ユーザー(手動のセレクション)あるいは[SELECT LIST ITEMS BY POSITION](select-list-items-by-position) または [SELECT LIST ITEMS BY REFERENCE](select-list-items-by-reference) コマンド(プログラムされたセレクション)によって*list* のリスト内で選択された、最初の項目の位置または参照を返します。
