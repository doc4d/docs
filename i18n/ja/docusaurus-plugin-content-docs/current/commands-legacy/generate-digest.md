...
**注:** パスワードの管理にMD5 および SHA アルゴリズムを使用することは推奨されていません。パスワードをチェックするためには、[Generate password hash](generate-password-hash) および [Verify password hash](verify-password-hash)コマンドを使用することが推奨されます。
