...
*data* の内容をエンコードしたい場合[BASE64 ENCODE](base64-encode) コマンドを使用しなければなりません。この場合もちろんBLOB を*data* に渡します。

[XML SET OPTIONS](xml-set-options)コマンドのXML String encoding オプションにをXML Raw data 渡すことでカレントプロセスにおいてこのメカニズムを無効にしない限り、デフォルトでコマンドは*data* 引数に含まれる特別文字 (< > ” ’...) をエンコードします。例:  
