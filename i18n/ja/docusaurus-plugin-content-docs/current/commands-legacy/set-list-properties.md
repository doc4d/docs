...
**注:** リスト内の各項目ごとのアイコンをカスタマイズしたい場合には、[SET LIST ITEM PROPERTIES](set-list-item-properties) コマンドを使用してください。
...
* プログラムを使用する場合、[SELECT LIST ITEMS BY POSITION](select-list-items-by-position) および[SELECT LIST ITEMS BY REFERENCE](select-list-items-by-reference) コマンドを使用します。  
