...
[SELECTION TO ARRAY](selection-to-array) または [SELECTION RANGE TO ARRAY](selection-range-to-array) のコマンドを Time 型のフィールドに対して適用した場合、これらのコマンドはコピー先の配列が他の型(例えば倍長整数など)に定義されていない場合に限り、時間型の配列を作成します。
