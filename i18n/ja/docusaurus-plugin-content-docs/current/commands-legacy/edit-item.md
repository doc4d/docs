...
* [DISPLAY SELECTION](display-selection) または [MODIFY SELECTION](modify-selection) で表示されたリストフォーム

コマンドがリストでない入力可オブジェクトに使用された場合、[GOTO OBJECT](goto-object) コマンドと同様に動作します。   
