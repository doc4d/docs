...
* *keyObject*: 暗号鍵を格納したJSON オブジェクト。<!-- END REF-->その構造は、[New data key](new-data-key) コマンドによって返されるオブジェクトと同じです。
...
復号化で使用する*passPhrase* 引数および *salt* 引数は、暗号化の際に使用された*passPhrase* 引数および *salt* 引数と同一です([Encrypt data BLOB](encrypt-data-blob) の例題を参照して下さい)。
