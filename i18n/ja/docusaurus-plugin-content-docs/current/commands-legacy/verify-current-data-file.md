...
このコマンドは、開かれたデータベースのカレントのデータファイルのみに適用されることを除き、[VERIFY DATA FILE](verify-data-file)コマンドと同じ機能を持ちます。そのため、ストラクチャとデータを指定する引数は必要ありません。  
引数の説明は[VERIFY DATA FILE](verify-data-file)コマンドを参照してください。
