...
<!--REF #_command_.QUERY SELECTION BY ATTRIBUTE.Summary-->**QUERY SELECTION BY ATTRIBUTE** は [QUERY BY ATTRIBUTE](query-by-attribute) と同じように動作します。<!-- END REF-->相違点は検索する範囲が異なるだけです:

* [QUERY BY ATTRIBUTE](query-by-attribute) はテーブルの全レコードからレコードを検索します。
...
詳細については [QUERY BY ATTRIBUTE](query-by-attribute) の説明を参照ください。

**QUERY SELECTION BY ATTRIBUTE** コマンドは、クエリが \* 引数で結合された [QUERY BY ATTRIBUTE](query-by-attribute) (および [QUERY](query)) 呼び出しのシークエンスを使用して定義することが出来ない場合に有用です。例えば、カレントセレクションがクエリではなく、[USE SET](use-set) のようなコマンドによって作成されている場合などが該当します。
