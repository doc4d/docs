...
サーバ上のカレントデータベースを閉じるには、サーバ上で**データベースを閉じる...** メニューコマンドを使用します。また**4D Serverを終了** メニューコマンドを選択したり、サーバ上で実行されるストアドプロシージャ内で[QUIT 4D](../commands/quit-4d.md "QUIT 4D") コマンドを呼び出すこともできます。
...
リモートの4Dがサーバへの接続を停止する時に、クライアントマシン上で自動的にコードを実行させたい場合には、[Semaphore](../commands/semaphore.md)を使用してください。
