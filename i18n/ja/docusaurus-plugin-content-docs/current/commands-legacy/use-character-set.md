...
<!--REF #_command_.USE CHARACTER SET.Summary-->**USE CHARACTER SET**は、データベースとドキュメントまたはシリアルポート間のデータ交換の際、4Dが 使用する文字セットを変更するために使用できます。<!-- END REF-->このコマンドはカレントプロセスに有効です。交換操作にはテキスト, DIF, そして SYLK ファイルの読み込み/書き出しが含まれます。文字マップはまた[SEND PACKET](send-packet), [RECEIVE PACKET](receive-packet) (テキスト型パケット) そして[RECEIVE BUFFER](receive-buffer)によるデータ交換にも使用されます。[SEND RECORD](send-record), [SEND VARIABLE](send-variable), [RECEIVE RECORD](receive-record), [SEND PACKET](send-packet), [RECEIVE PACKET](receive-packet) (BLOB型パケット) そして [RECEIVE VARIABLE](receive-variable) によるデータ交換には影響しません。
...
IANA名に関する説明は[CONVERT FROM TEXT](convert-from-text)コマンドにもあります。
