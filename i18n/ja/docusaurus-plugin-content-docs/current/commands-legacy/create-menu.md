...
* オプションの*menu*引数を渡さない場合、空のメニューが作成されます。[RELEASE MENU](release-menu) や[SET MENU ITEM](set-menu-item)などのコマンドを使用して、メニューを構築、管理しなくてはなりません。
...
このコマンドで作成されたメニューは、[SET MENU BAR](set-menu-bar) コマンドでメニューバーとして使用できます。

[Create menu](create-menu)で作成したメニューが必要なくなったときには、[RELEASE MENU](release-menu)コマンドを呼び出して使用されているメモリを解放してください。
