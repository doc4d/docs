...
<!--REF #_command_.BREAK LEVEL.Summary-->**BREAK LEVEL**は、[PRINT SELECTION](print-selection)コマンドを使ってプリントするレポートのブレークの数を指定します。<!-- END REF-->

ブレーク処理を行うレポートを印刷する前に、**BREAK LEVEL**コマンドと[ACCUMULATE](accumulate)コマンドを実行しなければなりません。これらのコマンドはレポートに対するブレーク処理を有効にします。[Subtotal](subtotal)コマンドの説明を参照してください。
...
以下の例は、2つのブレークレベルを持つレポートを印刷します。このセレクションは4つのレベルに対してソートされますが、**BREAK LEVEL**コマンドは2つのレベルだけにブレークを指定します。一つのフィールドは[ACCUMULATE](accumulate)コマンドで累計しています:
