...
* **データベースを最初に暗号化するときに限り、**渡すのは*newPassPhrase* あるいは *newDataKey* 引数だけです(逆に、*curPassPhrase* または *curDataKey* 引数は渡してはいけません):  
   ...
   * *newDataKey*: 新しいデータ暗号化キーを(*encodedKey* プロパティに)格納しているオブジェクト。このキーは[New data key](new-data-key) コマンドで生成されている必要があります。  
...
(\*) 検証ステップで返された警告([VERIFY DATA FILE](verify-data-file) コマンドを参照のこと)
