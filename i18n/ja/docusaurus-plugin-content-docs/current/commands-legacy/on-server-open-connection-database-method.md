...
* リモート4Dが、サーバー上でコオペラティブプロセスの作成を必要とする**\*** (プロセス名が"$"で始まらない) グローバルプロセスを開始した。このプロセスは[New process](new-process)コマンド、メニューコマンド、またはメソッド実行ダイアログボックスを使用して作成されます
