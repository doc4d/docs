...
4D では**LAUNCH EXTERNAL PROCESS** のコンテキストで使用可能で、[SET ENVIRONMENT VARIABLE](set-environment-variable) コマンドを使用して設定可能な3つの特定の環境変数を提供しています:
...
環境変数を、[SET ENVIRONMENT VARIABLE](set-environment-variable) コマンドを使用して"false" に設定した場合(非同期実行)、*outputStream* および *errorStream* 引数は返されません。
