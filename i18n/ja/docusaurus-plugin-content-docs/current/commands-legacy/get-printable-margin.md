...
<!--REF #_command_.GET PRINTABLE MARGIN.Summary-->GET PRINTABLE MARGIN コマンドは、[Print form](../commands/print-form)、[PRINT SELECTION](print-selection) そして [PRINT RECORD](print-record) コマンドに使用されるマージンの現在値を返します。<!-- END REF--> 
...
4Dバージョン6.8.1以降、[Print form](../commands/print-form)、[PRINT SELECTION](print-selection)および[PRINT RECORD](print-record) コマンドを使用して行うフォームの印刷を、各プリンタ共通の固定マージン（用紙マージン、つまり用紙の物理的な許容限界寸法）に基づいて行うことができるようになりました。これを行うには、 [GET PRINTABLE MARGIN](get-printable-margin)、 [SET PRINTABLE MARGIN](set-printable-margin) および[GET PRINTABLE AREA](get-printable-area) コマンドを使用します。
