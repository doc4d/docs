...
* *keyObject*: 暗号鍵を格納したJSON オブジェクト。その構造は、[New data key](new-data-key) コマンドによって返されるオブジェクトと同じです。
