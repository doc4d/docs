...
メニューが生成されたら、4Dの [Dynamic pop up menu](dynamic-pop-up-menu) コマンドで表示し、4Dの標準メニュー管理コマンドを使用して処理できます。このコマンドから返される文字列には、訪問したページのURLが含まれます (例題参照)。

メニューを使用しなくなったら、[RELEASE MENU](release-menu) コマンドを呼び出してURL履歴メニューを削除します。
