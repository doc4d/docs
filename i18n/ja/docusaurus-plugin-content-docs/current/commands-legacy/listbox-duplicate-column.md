...
* コレクションまたはエンティティセレクション型のリストボックスの場合、*colVariable* にNil ポインター(->\[\])を渡します。その後、データソース式を指定するために[LISTBOX SET COLUMN FORMULA](listbox-set-column-formula) コマンドを呼び出す必要があります。
