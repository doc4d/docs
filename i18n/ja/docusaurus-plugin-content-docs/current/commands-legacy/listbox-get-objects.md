...
このコマンドは [FORM LOAD](../commands/form-load)、[FORM GET OBJECTS](form-get-objects) と [OBJECT Get type](object-get-type) コマンドを使用してフォームを解析する際に有用です。必要に応じてリストボックスのサブオブジェクトの名前を取得するために使用することができます。
