...
このファンクションは*password* 引数のパスワードを、[Generate password hash](generate-password-hash) ファンクションで生成された*hash* 引数のハッシュと比較します。
...
この例題では[Generate password hash](generate-password-hash) コマンドで作成済みで\[Users\] テーブルに保存されているパスワードハッシュを、新たに入力されたパスワードと検証する場合を考えます:   
