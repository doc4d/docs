...
使用できるプリンターの一覧および追加情報を取得するには[PRINTERS LIST](printers-list)コマンドを使用します。カレントプリンタを変更するには、[SET CURRENT PRINTER](set-current-printer)を使用します。

**注:** [SET CURRENT PRINTER](set-current-printer) で Generic PDF driver のオプションを有効にしている場合、[Get current printer](get-current-printer) コマンドの戻り値は "\_4d\_pdf\_printer" または実際の PDF ドライバーの名称です。
