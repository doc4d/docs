...
スタイル、カラー、背景色、または行管理配列は、デザインモードのプロパティリスト、あるいは [LISTBOX SET ARRAY](listbox-set-array) コマンドを使用することで、配列型のリストボックス、または (行管理配列以外のみ) 配列型リストボックスのカラムと関連付けすることができます。
...
* 要求されたプロパティの配列がそのリストボックスまたは列に関連付けされていない場合、[Is nil pointer](is-nil-pointer)が返ってきます。
* ユーザーによって定義されたプロパティの配列へのポインター
* [LISTBOX SET ROW COLOR](listbox-set-row-color) や [LISTBOX SET ROW FONT STYLE](listbox-set-row-font-style) コマンドを使用して設定されたプロパティの配列へのポインター
