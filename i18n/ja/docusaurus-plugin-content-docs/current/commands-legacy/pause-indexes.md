...
オペレーションが終了した後にインデックスを再開させるためには、 *aTable* に [RESUME INDEXES](resume-indexes) コマンドをしようします。 

**注:** [CREATE INDEX](create-index) コマンドと [DELETE INDEX](delete-index) コマンドを使用することによって似たような結果を得ることが出来ますが、そこには大きな違いがあります:

* *aTable* のそれぞれのインデックスに対して[DELETE INDEX](delete-index) / [CREATE INDEX](create-index) コマンドを呼び出す必要があります。
* [DELETE INDEX](delete-index) / [CREATE INDEX](create-index) コマンドを呼び出すと、インデックスの内部番号が変わりますが、**PAUSE INDEXES** / [RESUME INDEXES](resume-indexes) ではその限りではありません。インデックス番号が変わると、データセットを変更するときにデータの自動再インデックスが生成されます。

**PAUSE INDEXES** コマンドをテーブルに使用し、 [RESUME INDEXES](resume-indexes) コマンドをそのテーブルに使用することなくデータベースを閉じた場合、このテーブルのインデックスはデータベースが再開したときに全て自動的にリビルドされます。
