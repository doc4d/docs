...
`#DECLARE` キーワードを使用すると、パラメーターは自動的に宣言されます。 例:
...
> 任意パラメーターが必要な場合、[オブジェクトプロパティを名前付き引数として使用する](#using-objects-properties-as-named-parameters) と型の制限がなく、柔軟で便利です。
...
このようにサブルーチンの戻り値を使うことを "関数を使う" と言います。 詳細については [戻り値](#returning-values) の章を参照ください。
