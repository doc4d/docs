...
このメニューは `formMacros.json` [マクロ定義ファイル](#location-of-macros) をもとに作成されています。 マクロメニュー項目は ABC順に表示されます。
...
クラスの名称は、`formMacros.json` ファイルで [class](#creating-macros) 属性に定義した名前と同一でなくてはなりません。
