...
未定義 (undefined) は、実際にはデータタイプではありません。 未定義は、まだ定義されていない変数を示します。 Evaluating an object property can also produce an undefined value. 未定義の値のプロパティを読み取ると、**undefined** が返されます。
