...
| 戻り値      | 4D.EntitySelection |  <- | Entity selection matching the dataclass related to the given table | <!-- END REF --> |
...
| バージョン  | 内容                                     |
| ------ | -------------------------------------- |
| v19 R7 | Support of _entitySelection_ parameter |
...
| 戻り値             | 4D.EntitySelection |  -> | Entity selection including the added _entity_ or _entitySelection_ | <!-- END REF --> |
...
| 戻り値             | 4D.EntitySelection |  <- | New entity selection with the result of intersection with logical AND operator | <!-- END REF --> |
...
| 戻り値           | Real |  <- | Arithmetic mean (average) of entity attribute values (Undefined if empty entity selection) | <!-- END REF --> |
...
| 戻り値    | Boolean   |  <- | True if the entity belongs to the entity selection, else False | <!-- END REF --> |
...
| ------------- | ---- | :-: | ----------------------------------------------------------------- | ---------------- |
| attributePath | Text |  -> | 計算に使用する属性パス                                                       |                  |
| 戻り値           | Real |  <- | Number of non null _attributePath_ values in the entity selection | <!-- END REF --> |
...
| 戻り値    | 4D.EntitySelection |  <- | Copy of the entity selection                     | <!-- END REF --> |
...
| 戻り値           | Collection |  <- | Collection with only distinct values | <!-- END REF --> |
...
| 戻り値           | Collection |  <- | Collection containing extracted values                                                                     | <!-- END REF --> |
...
| 引数  | タイプ       |     | 説明                                                                                                    |                  |
| --- | --------- | :-: | ----------------------------------------------------------------------------------------------------- | ---------------- |
| 戻り値 | 4D.Entity |  <- | Reference to the first entity of the entity selection (Null if selection is empty) | <!-- END REF --> |
...
| 引数  | タイプ          |     | 説明                                                     |                  |
| --- | ------------ | :-: | ------------------------------------------------------ | ---------------- |
| 戻り値 | 4D.DataClass |  <- | Dataclass object to which the entity selection belongs | <!-- END REF --> |
...
| 引数  | タイプ     |     | 説明                                                         |                  |
| --- | ------- | :-: | ---------------------------------------------------------- | ---------------- |
| 戻り値 | Boolean |  <- | True if the entity selection is alterable, False otherwise | <!-- END REF --> |
...
| 引数  | タイプ     |     | 説明                                                       |                  |
| --- | ------- | :-: | -------------------------------------------------------- | ---------------- |
| 戻り値 | Boolean |  <- | True if the entity selection is ordered, False otherwise | <!-- END REF --> |
...
| 引数  | タイプ       |     | 説明                                                                                                       |                  |
| --- | --------- | :-: | -------------------------------------------------------------------------------------------------------- | ---------------- |
| 戻り値 | 4D.Entity |  <- | Reference to the last entity of the entity selection (Null if empty entity selection) | <!-- END REF --> |
...
| ------------- | ---- | -- | -------------------------- | ---------------- |
| attributePath | Text | -> | 計算に使用する属性パス                |                  |
| 戻り値           | any  | <- | Highest value of attribute | <!-- END REF --> |
...
| ------------- | ---- | :-: | ------------------------- | ---------------- |
| attributePath | Text |  -> | 計算に使用する属性パス               |                  |
| 戻り値           | any  |  <- | Lowest value of attribute | <!-- END REF --> |
...
| バージョン  | 内容                               |
| ------ | -------------------------------- |
| v19 R7 | Support of _keepOrder_ parameter |
| v17    | 追加                               |
...
| 戻り値             | 4D.EntitySelection |  <- | New entity selection or a new reference on the existing entity selection                                   | <!-- END REF --> |
...
| entity          | 4D.Entity          |  -> | 交差するエンティティ                                                             |                  |
| entitySelection | 4D.EntitySelection |  -> | 交差するエンティティセレクション                                                       |                  |
| 戻り値             | 4D.EntitySelection |  <- | New entity selection or new reference to the original entity selection | <!-- END REF --> |
...
| 戻り値         | 4D.EntitySelection |  <- | New entity selection in the specified order | <!-- END REF --> |
...
| 戻り値           | 4D.EntitySelection |  <- | New ordered entity selection                                   | <!-- END REF --> |
...
| 戻り値           | 4D.EntitySelection |  <- | New entity selection made up of entities from entity selection meeting the search criteria specified in _queryString_ or _formula_ | <!-- END REF --> |
...
| 戻り値              | Object             |  <- | Range(s) of selected entities in entity selection | <!-- END REF --> |
...
| 戻り値       | 4D.EntitySelection |  <- | New entity selection containing sliced entities (shallow copy) | <!-- END REF --> |
...
| ------------- | ---- | :-: | ------------------------------ | ---------------- |
| attributePath | Text |  -> | 計算に使用する属性パス                    |                  |
| 戻り値           | Real |  <- | Sum of entity selection values | <!-- END REF --> |
...
| 戻り値          | Collection |  <- | Collection of objects containing attributes and values of entity selection      | <!-- END REF --> |
...
filterString および filterCol 引数が省略されるか、空の文字列が渡されるか、あるいは "\*" が渡された場合、すべての属性が抽出されます。 Attributes with [kind](DataClassClass.md#attributename) property as "relatedEntity" are extracted with the simple form: an object with property __KEY (primary key). "relatedEntities" 型の "kind" プロパティの属性は抽出されません。
