...
| keywords | Result | -> | 追加するキーワードフラグ                                                                                                |
| Result   | Result | <- | addFlags処理のステータス|<!-- END REF -->
...
| 引数             | タイプ    |    | 詳細                                        |
| -------------- | ------ |:--:| ----------------------------------------- |
| mailObj        | Result | -> | Email オブジェクト                              |
| destinationBox | テキスト   | -> | Emailオブジェクトを受信するメールボックス                   |
| options        | Result | -> | 文字セット情報を格納したオブジェクト                        |
| Result         | Result | <- | append処理のステータス|<!-- END REF -->
...
| Result         | Result | <- | copy処理のステータス|<!-- END REF -->
...
| Result | Result | <- | Status of the mailbox creation operation|<!-- END REF -->
...
| msgsIDs | コレクション | -> | メッセージの固有ID のコレクション (テキスト)                                 |
| allMsgs | 整数     | -> | `IMAP all`: 選択されたメールボックスの全メッセージ                           |
| Result  | Result | <- | Status of the delete operation|<!-- END REF -->
...
| 引数     | タイプ    |    | 詳細                                                          |
| ------ | ------ |:--:| ----------------------------------------------------------- |
| Result | Result | <- | Status of the expunge operation |<!-- END REF -->
...
| Result | Result | <- | boxInfo object|<!-- END REF -->
...
| mailRecent | 数値   | (新しいメッセージであることを表す) "recent" フラグがついたメッセージの数 |
...
| options   | Result | -> | メッセージ管理オプション                                                                |
| Result    | Result | <- | [Email object](EmailObjectClass.md#email-object)|<!-- END REF -->
...
| options  | Result | -> | メッセージ管理オプション                                                                         |
| Result   | Result | <- | 次のコレクションを格納したオブジェクト:<br/><ul><li>[Email オブジェクト](EmailObjectClass.md#email-オブジェクト) のコレクション</li><li>見つからなかったメッセージの ID または番号のコレクション</li></ul>|<!-- END REF -->
...
| Result         | Result | <- | Status of the move operation|<!-- END REF -->
...
| -------- | ------ |:--:| ----------------------------------------------------------------------------------------------------------- |
| msgIDs   | コレクション | -> | 文字列のコレクション: メッセージの固有ID (テキスト型)<br/> テキスト: メッセージの固有ID<br/> 倍長整数 (IMAP all): 選択されたメールボックス内の全メッセージ |
| keywords | Result | -> | 削除するキーワードフラグ                                                                                                |
| Result   | Result | <- | Status of the removeFlags operation|<!-- END REF -->
...
| Result      | Result | <- | Status of the renaming operation|<!-- END REF -->
...
| name       | テキスト | メールボックスの名称              |
| mailCount  | 数値   | メールボックス内のメッセージの数        |
| mailRecent | 数値   | "recent" フラグがついたメッセージの数 |
...
| ------ | ------ |:--:| ------------------------------------------------------------ |
| name   | テキスト   | -> | メールボックスの名称                                                   |
| Result | Result | <- | Status of the subscribe operation|<!-- END REF -->
...
| ------ | ------ |:--:| -------------------------------------------------------------- |
| name   | テキスト   | -> | メールボックスの名称                                                     |
| Result | Result | <- | Status of the unsubscribe operation|<!-- END REF -->
