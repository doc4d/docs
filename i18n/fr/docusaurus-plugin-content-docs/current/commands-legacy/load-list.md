...
Pour connaître les énumérations définies dans la base, utilisez la commande [LIST OF CHOICE LISTS](list-of-choice-lists). Pour savoir si la liste a correctement été chargée, utilisez la fonction [Is a list](is-a-list) avec le numéro de référence retourné par **Load list**.
...
Si vous modifiez la liste nouvellement créée et voulez enregistrer ces modifications, utilisez la commande [SAVE LIST](save-list). 

Si vous n'avez plus besoin de la liste, n'oubliez pas d'appeler [CLEAR LIST](clear-list) pour la supprimer. Sinon, elle reste en mémoire jusqu'à la fin de la session de travail ou jusqu'à ce que le process dans lequel la liste a été créée soit détruit.

**Astuce :** Si vous associez une liste à un objet de formulaire (liste hiérarchique, onglet ou menu hiérarchique) à l'aide du menu **Enumération** dans la Liste des propriétés, il est inutile d'appeler **Load list** ou [CLEAR LIST](clear-list) dans la méthode de l'objet. 4D charge et efface la liste automatiquement pour vous.
