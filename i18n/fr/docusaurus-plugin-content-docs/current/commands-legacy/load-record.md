...
Il est alors utile d'appeler la fonction [Locked](locked) pour déterminer si l'enregistrement peut être modifié :
...
* Si la table est en mode Lecture/écriture et si l'enregistrement n'est pas verrouillé, vous pouvez le modifier dans le process courant. La fonction [Locked](locked) retournera Vrai pour tous les autres utilisateurs et process.

**Note :** Si la commande **LOAD RECORD** est exécutée après un [READ ONLY](read-only), l'enregistrement est automatiquement libéré et chargé, sans qu'il soit nécessaire d'appeler la commande [UNLOAD RECORD](unload-record).

Vous n'aurez normalement pas besoin d'appeler la commande **LOAD RECORD**, car toutes les commandes telles que [QUERY](query), [NEXT RECORD](next-record), [PREVIOUS RECORD](previous-record), etc., chargent automatiquement l'enregistrement courant.
