...
Le menu hiérarchique utilisé doit avoir été créé à l’aide de la commande [Create menu](create-menu). La référence retournée par [Create menu](create-menu) doit être passée dans le paramètre *menu*.

**Note :** La commande [Pop up menu](pop-up-menu) (thème “Interface utilisateur”) permet de créer des pop up menus basés sur du texte.
...
Si une ligne de menu a été sélectionnée, la commande retourne sa chaîne de caractères personnalisée associée (telle que définie à l’aide de la commande [SET MENU ITEM PARAMETER](set-menu-item-parameter)). Sinon, la commande retourne une chaîne vide.
...
* Si une action standard n'est pas activée (c'est-à-dire ne peut pas être appelée) dans le contexte du pop-up menu, le libellé est automatiquement caché. Vous pouvez savoir si une action est disponible en utilisant la commande [Action info](action-info).
