...
* N’appelez aucun élément d’interface dans la **On Web Authentication database method** (([ALERT](alert), [DIALOG](../commands/dialog), etc.), sinon son exécution sera interrompue et la connexion refusée. Il en est de même si une erreur se produit durant son traitement.
...
* lorsque 4D reçoit un URL d'accès à la racine et qu'aucune page d'accueil n'est définie dans les propriétés de la base ou via la commande [WEB SET HOME PAGE](web-set-home-page)
