...
* Si vous ne passez pas le paramètre facultatif *menu*, le menu sera créé vide. Vous devrez le construire et le gérer à l’aide des commandes [RELEASE MENU](release-menu), [SET MENU ITEM](set-menu-item), etc.
...
Un menu créé par cette commande peut être utilisé en tant que barre de menus à l’aide de la commande [SET MENU BAR](set-menu-bar).

Lorsque vous n'avez plus besoin d'un menu créé par [Create menu](create-menu), n'oubliez pas d'appeler la commande [RELEASE MENU](release-menu) afin de libérer la mémoire qu'il occupe. 

## Exemple 

Reportez-vous à l'exemple de la commande [SET MENU BAR](set-menu-bar).
