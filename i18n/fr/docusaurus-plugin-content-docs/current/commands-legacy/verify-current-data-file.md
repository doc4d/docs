...
Cette commande a un fonctionnement identique à celui de la commande [VERIFY DATA FILE](verify-data-file), à la différence près qu’elle s’applique uniquement au fichier de données courant de la base de données ouverte. Elle ne nécessite donc pas de paramètres désignant la structure et les données.  
Reportez-vous à la commande [VERIFY DATA FILE](verify-data-file) pour la description des paramètres.
...
[VERIFY DATA FILE](verify-data-file)  
