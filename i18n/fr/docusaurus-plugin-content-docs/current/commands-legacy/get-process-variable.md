...
Attention, la communication process “intermachine” permise par les commandes **GET PROCESS VARIABLE**, [SET PROCESS VARIABLE](set-process-variable) et [VARIABLE TO VARIABLE](variable-to-variable) n’est possible que du client vers le serveur. C’est toujours un process client qui lit ou écrit les variables d’une procédure stockée.
...
**Note :** En mode interprété, si une variable source n'existe pas, la valeur indéfinie est retournée. Vous pouvez le détecter en testant la variable de destination correspondante à l'aide de la fonction [Type](type). 
