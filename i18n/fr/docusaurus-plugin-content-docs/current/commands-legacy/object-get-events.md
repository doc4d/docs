...
Les événements formulaire peuvent avoir été activés/désactivés soit via la Liste des propriétés, soit via la commande [OBJECT SET EVENTS](object-set-events) si elle a été appelée dans le process courant.
...
[OBJECT SET EVENTS](object-set-events)  
