...
| contextAttributes | Objet         | Objet contenant des informations semblables à la commande [LOCKED BY](locked-by) mais appliquées à l'enregistrement, à la différence près que **Locked records info** retourne uniquement le nom de l'utilisateur défini dans le système (et pas celui de l'utilisateur 4D) ainsi que des informations supplémentaires (voir ci-dessous). |
...
| user4d\_alias                                                                                                                    | texte         | Alias utilisateur défini avec [SET USER ALIAS](set-user-alias), sinon le nom d'utilisateur dans le répertoire de la base 4D |
