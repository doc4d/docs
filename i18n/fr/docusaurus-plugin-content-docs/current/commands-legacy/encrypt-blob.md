...
Après l’exécution de la commande, le BLOB *aCrypter* contient les données cryptées. Ces données ne pourront être décryptées qu’avec la commande [DECRYPT BLOB](decrypt-blob), à laquelle la clé publique de l’émetteur sera passée en paramètre. En outre, si la clé publique (optionnelle) du récepteur avait été utilisée pour le cryptage, la clé privée du récepteur sera également nécessaire pour le décryptage.
...
1\. La société commence par générer une paire de clés à l’aide de la commande [GENERATE ENCRYPTION KEYPAIR](generate-encryption-keypair). 
