...
Passez dans *refElément* la référence de l’élément racine (créé par exemple à l’aide de la commande [DOM Create XML Ref](dom-create-xml-ref)).
...
**Note :** Si vous avez défini un ou plusieurs espace(s) de nommage pour l’arbre désigné par *refElément* (cf. commande [DOM Create XML Ref](dom-create-xml-ref)), vous devez préfixer le paramètre *xPath* du nom de l’espace à utiliser (par exemple “MonNameSpace:MonElément”).
...
[DOM Create XML element arrays](dom-create-xml-element-arrays)  
