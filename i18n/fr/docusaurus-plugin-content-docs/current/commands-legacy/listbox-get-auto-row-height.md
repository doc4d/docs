...
La valeur courante de hauteur minimum ou maximum peut être définie soit dans la Liste des propriétés (voir *Hauteur de ligne automatique*) soit, pour le process courant, à l'aide de la commande [LISTBOX SET AUTO ROW HEIGHT](listbox-set-auto-row-height).
