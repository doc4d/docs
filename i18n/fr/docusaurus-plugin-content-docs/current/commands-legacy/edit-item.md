...
* Formulaires liste affichés via la commande [DISPLAY SELECTION](display-selection) ou [MODIFY SELECTION](modify-selection).

Si la commande est utilisée avec un objet saisissable qui n’est pas une liste, elle équivaut à la commande [GOTO OBJECT](goto-object).   
