...
* Si la valeur de *positionLigne* est supérieure au nombre total de lignes contenues dans la listbox (dans le cas d'une listbox de type tableau uniquement), le tableau booléen associé à la listbox est automatiquement redimensionné et l’action de sélection est effectuée. Ce mécanisme permet d’utiliser **LISTBOX SELECT ROW** avec des commandes “standard” de gestion de tableaux (telles que [APPEND TO ARRAY](append-to-array)) n’entraînant pas de synchronisation immédiate de la listbox.  
...
* Si vous souhaitez que la list box défile de manière à afficher la ligne nouvellement sélectionnée, utilisez la commande [OBJECT SET SCROLL POSITION](object-set-scroll-position).
* Pour passer une ligne en mode édition (saisie), utilisez la commande [EDIT ITEM](edit-item).
...
[LISTBOX SELECT BREAK](listbox-select-break)  
[LISTBOX SELECT ROWS](listbox-select-rows)  
