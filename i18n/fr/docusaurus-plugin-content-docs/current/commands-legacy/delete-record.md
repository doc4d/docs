...
* Si l'enregistrement courant est déchargé de la mémoire avant l'appel à **DELETE RECORD** (par exemple suite à un [UNLOAD RECORD](unload-record)), la sélection courante de *laTable* est vide à l'issue de la suppression.
* La commande **DELETE RECORD** ne fait rien si la table est en mode [READ ONLY](read-only), indépendamment de l'état verrouillé ou non de l'enregistrement à supprimer.
