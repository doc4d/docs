...
**Note :** Il est possible de modifier l’icône associée à un élément à l’aide de la commande [SET LIST ITEM PROPERTIES](set-list-item-properties). Toutefois, [SET LIST ITEM PROPERTIES](set-list-item-properties) accepte uniquement des références d’images statiques (références de ressources ou images de la bibliothèque).
...
Vous pouvez passer un numéro de référence dans *réfElément*. Si ce numéro ne correspond à aucun élément de la liste, la commande ne fait rien. Vous pouvez également passer 0 dans *réfElément* afin de demander la modification du dernier élément ajouté à la liste (à l’aide de [APPEND TO LIST](append-to-list)).  
