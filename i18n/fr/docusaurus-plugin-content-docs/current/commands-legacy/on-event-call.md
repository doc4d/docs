...
* La variable système KeyCode contient le code du caractère tapé au clavier, ou le code d'une touche de fonction. Référez-vous aux sections *Codes Unicode* and [EXPORT TEXT](export-text) qui listent les codes de caractères utilisés par 4D, ainsi qu'à la section *Codes des touches de fonction*. 4D fournit des constantes prédéfinies pour les principaux codes ASCII et touches de fonctions. Vous pouvez les visualiser à l'aide la fenêtre de l'Explorateur, dans les thèmes correspondants.
...
Notez que **ON EVENT CALL** est utilisé dans cet exemple car un état spécial est imprimé à l'aide des commandes *\_o\_PAGE SETUP*, [Print form](../commands/print-form) et [PAGE BREAK](page-break) dans une structure de type [ARRAY TO LIST](array-to-list).

Lorsque vous imprimez un état à l'aide la commande [PRINT SELECTION](print-selection), vous n'avez pas besoin de gérer les événements permettant à l'utilisateur d'interrompre l'impression, [PRINT SELECTION](print-selection) le fait pour vous.
