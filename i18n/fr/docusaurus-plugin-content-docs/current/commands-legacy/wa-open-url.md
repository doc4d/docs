...
Comme la commande [OPEN URL](open-url), **WA OPEN URL** accepte plusieurs types de syntaxes dans le paramètre *url* pour désigner les fichiers :
...
Sous Mac OS, quand FileVault est activé, vous devez utiliser la syntaxe posix. Vous pouvez transformer les chemins système via la commande [Convert path system to POSIX](convert-path-system-to-posix). 
