...
* Lorsque l'éditeur est appelé via la commande **QR REPORT**, les liens entre les tables conservent leur statut manuel, le cas échéant. Ce principe permet au développeur de gérer lui-même ce statut à l'aide des commandes [SET AUTOMATIC RELATIONS](set-automatic-relations) et [SET FIELD RELATION](set-field-relation). Toutefois, n'oubliez pas que les automatismes sont désactivés lorsque les tables liées sont visualisées dans un formulaire liste affiché à l'aide de [DISPLAY SELECTION](display-selection), [MODIFY SELECTION](modify-selection) ou d'un sous-formulaire. Voir *Liens manuels et automatiques*.
* L'éditeur est appelé dans une fenêtre externe, il n'est pas possible d'utiliser la commande [QR ON COMMAND](qr-on-command) dans ce contexte. Vous pouvez cependant utiliser le paramètre *nomMéthode* afin d'exécuter du code personnalisé lorsqu'une commande d'interface est activée (cf. ci-dessous).
...
Reportez-vous à l'exemple de la commande [SET FIELD RELATION](set-field-relation). 
