...
<!--REF #_command_.GET TABLE TITLES.Summary-->La commande **GET TABLE TITLES** remplit les tableaux *titresTables* et *numTables* avec les noms et les numéros des tables de la base définis dans la fenêtre de Structure ou via la commande [SET TABLE TITLES](set-table-titles).<!-- END REF--> Le contenu des deux tableaux est synchronisé.

Si la commande [SET TABLE TITLES](set-table-titles) a été appelée lors de la session, **GET TABLE TITLES** retourne uniquement les noms “modifiés” et les numéros des tables ayant été définies via cette commande.   
