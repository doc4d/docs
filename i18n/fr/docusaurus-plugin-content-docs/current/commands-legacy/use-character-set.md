...
<!--REF #_command_.USE CHARACTER SET.Summary-->La commande **USE CHARACTER SET** permet de modifier le jeu de caractères utilisé par 4D pour toutes les opérations de transfert entre la base et un document ou un port série pour le process courant.<!-- END REF--> Cela inclut les données transférées par les commandes d'import/export Texte, SYLK et DIF, ainsi que celles envoyées par les commandes [SEND PACKET](send-packet) et [RECEIVE PACKET](receive-packet) (paquets de type texte) et [RECEIVE BUFFER](receive-buffer). Les filtres n'ont pas d'effet sur les données transférées par les commandes [SEND RECORD](send-record), [SEND VARIABLE](send-variable), [RECEIVE RECORD](receive-record), [SEND PACKET](send-packet), et [RECEIVE PACKET](receive-packet) (paquets de type BLOB) et [RECEIVE VARIABLE](receive-variable).
