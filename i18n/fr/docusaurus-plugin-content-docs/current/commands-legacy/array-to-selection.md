...
Cette commande effectue l'opération inverse de [SELECTION TO ARRAY](selection-to-array). Cependant, **ARRAY TO SELECTION** ne permet pas d'utiliser de champs en provenance de tables différentes ni de tables liées, même si un lien automatique existe.
...
A l’image de la commande [QUERY](query), ce principe vous permet de scinder une instruction complexe en un ensemble de lignes, plus lisibles et plus faciles à maintenir. Il est également possible d’insérer des instructions intermédiaires. 
