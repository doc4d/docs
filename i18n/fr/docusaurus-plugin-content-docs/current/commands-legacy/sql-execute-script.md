...
**Note :** Les commandes [SQL EXPORT DATABASE](sql-export-database) et [SQL EXPORT SELECTION](sql-export-selection) génèrent automatiquement ce fichier de script.
...
Si le fichier d’enregistrement des requêtes de 4D est activé (via les sélecteurs 28 ou 45 de la commande [SET DATABASE PARAMETER](set-database-parameter)), chaque commande SQL exécutée génèrera une entrée avec les informations suivantes :
