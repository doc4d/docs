...
Le nouvel objet peut être déplacé et redimensionné via les paramètres *dépH*, *dépV*, *redimH* et *redimV*. Comme pour la commande [OBJECT MOVE](object-move), le sens du déplacement ou du redimensionnement est défini par le signe des valeurs passées dans les paramètres *dépH* et *dépV* :
...
* Evénement On Unload.
* Evénement lié à un contexte d’impression (On Header, On Printing Detail, etc.). Pour imprimer plusieurs fois un objet, vous devez utiliser la commande [Print object](print-object).
