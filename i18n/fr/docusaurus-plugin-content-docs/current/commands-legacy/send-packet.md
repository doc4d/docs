...
**Note :** Lorsque vous passez un BLOB dans *paquet*, la commande ne tient pas compte du jeu de caractères éventuellement défini par la commande [USE CHARACTER SET](use-character-set). Le BLOB est envoyé sans aucune modification.

Avant d'utiliser **SEND PACKET**, vous devez ouvrir un port série ou un document avec la commande [SET CHANNEL](set-channel), ou un document avec une commande de gestion des documents.
...
**Note :** Ce fonctionnement est valide avec un document ouvert par [SET CHANNEL](set-channel). Cependant, pour un document ouvert par [Open document](open-document), [Create document](create-document) ou [Append document](append-document), vous pouvez utiliser les commandes [Get document position](get-document-position) et [SET DOCUMENT POSITION](set-document-position) pour connaître et modifier la position à laquelle, dans le document, la prochaine écriture (**SEND PACKET**) ou lecture ([RECEIVE PACKET](receive-packet)) aura lieu.
