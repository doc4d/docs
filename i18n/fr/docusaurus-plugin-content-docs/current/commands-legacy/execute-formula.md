...
* Si *instruction* est une méthode projet, il est recommandé d'utiliser [EXECUTE METHOD](execute-method) qui permet de passer des paramètres.
...
La taille du cache est de zéro par défaut (pas de cache) ; vous devez le créer et l'ajuster à l'aide de la commande [SET DATABASE PARAMETER](set-database-parameter). Par exemple :
