...
Une fois le menu généré, vous pouvez l’afficher via la commande de 4D [Dynamic pop up menu](dynamic-pop-up-menu) et le manipuler via les commandes standard de gestion des menus de 4D. La chaîne retournée par la commande [Dynamic pop up menu](dynamic-pop-up-menu) contient l’URL de la page visitée (voir exemple).

Appelez la commande [RELEASE MENU](release-menu) pour supprimer un menu historique d’URL lorsqu’il est devenu inutile.
