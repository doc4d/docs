...
<!--REF #_command_.GOTO RECORD.Summary-->**GOTO RECORD** sélectionne l'enregistrement courant de *table*.<!-- END REF--> Le paramètre *enregistrement* est le numéro renvoyé par la fonction [Record number](record-number). Après l'exécution de cette commande, l'enregistrement est le seul de la sélection courante.
...
Référez-vous à l'exemple de la commande [Record number](record-number).
