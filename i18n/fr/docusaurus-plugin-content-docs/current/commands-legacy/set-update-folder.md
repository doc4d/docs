...
Si les paramètres sont valides, la mise à jour est placée "en attente" dans la session jusqu’à l’appel de la commande [RESTART 4D](restart-4d). Si vous exécutez plusieurs fois **SET UPDATE FOLDER** avant [RESTART 4D](restart-4d), le dernier appel valide est pris en compte. 
...
Exception : s'il n'est pas possible de créer un fichier journal, une boîte de dialogue d’alerte est affichée, quelle que soit la valeur du paramètre *erreursDiscrètes*. Pour plus d'informations, reportez-vous à la description de la commande [Get last update log path](last-update-log-path).
