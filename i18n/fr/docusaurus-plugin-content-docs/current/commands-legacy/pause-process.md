...
<!--REF #_command_.PAUSE PROCESS.Summary-->**PAUSE PROCESS** suspend l'exécution de *process* jusqu'à ce qu'il soit remis en action par la comande [RESUME PROCESS](resume-process).<!-- END REF--> Pendant ce temps, *process* n'utilise pas de temps machine. Lorsqu'un process est suspendu, il existe toujours en mémoire.

Si *process* est déjà suspendu, **PAUSE PROCESS** ne fait rien. Si le process est endormi à l'aide de [DELAY PROCESS](delay-process), le process est suspendu. S'il reçoit l'ordre [RESUME PROCESS](resume-process), le process redevient actif immédiatement.
