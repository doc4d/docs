...
4D vous fournit des actions automatiques pour les boutons qui effectuent les mêmes tâches que les commandes [FORM FIRST PAGE](form-first-page), [FORM LAST PAGE](form-last-page), [FORM NEXT PAGE](form-next-page), [FORM PREVIOUS PAGE](form-previous-page), [FORM GOTO PAGE](form-goto-page) que vous pouvez associer aux objets tels que les onglets, les listes déroulantes, etc. A chaque fois que c'est possible, utilisez les actions automatiques pour les boutons plutôt que ces commandes.   
...
* Vous pouvez utiliser la commande [FORM Get current page](form-get-current-page) pour savoir quelle page est affichée à l'écran.
