...
Lorsqu'un BLOB a été compressé, vous pouvez le décompresser à l'aide de la commande [EXPAND BLOB](expand-blob).

Pour savoir si un BLOB a été compressé, utilisez la commande [BLOB PROPERTIES](blob-properties).

**ATTENTION :** Un BLOB compressé est toujours un BLOB, rien ne vous empêche donc de modifier son contenu. Cependant, si vous le modifiez, la commande [EXPAND BLOB](expand-blob) ne pourra plus décompresser correctement le BLOB.
