...
Si *docRef* est spécifié, la commande récupère des données depuis un document ouvert par la fonction [Open document](open-document), [Create document](create-document) ou [Append document](append-document). Si *docRef* est omis, la commande récupère des données depuis un port série ou un document ouvert par la commande [SET CHANNEL](set-channel). 

Dans tous les cas, les caractères lus sont retournés dans la variable *réceptVar*, qui doit être une variable de type Texte, Alpha ou BLOB. Si les données ont été envoyées par la commande [SEND PACKET](send-packet), le type doit correspondre à celui du paquet envoyé.

**Notes**

* Si le paquet reçu est de type BLOB, la commande ne tient pas compte du jeu de caractères éventuellement défini par la commande [USE CHARACTER SET](use-character-set). Le BLOB est retourné sans aucune modification.
...
* lorsque **RECEIVE PACKET** lit des données en provenance du port série, la commande s'exécutera indéfiniment jusqu'à ce que le délai d'attente (s'il est fixé) soit écoulé (cf. la commande [SET TIMEOUT](set-timeout)) ou que l'utilisateur interrompe la réception (voir ci-dessous).
...
**Note :** Ce fonctionnement est valide avec un document ouvert par [SET CHANNEL](set-channel). Cependant, pour un document ouvert par [Open document](open-document), [Create document](create-document) ou [Append document](append-document), vous pouvez aussi utiliser les commandes [Get document position](get-document-position) et [SET DOCUMENT POSITION](set-document-position)pour connaître et modifier la position à laquelle, dans le document, la prochaine écriture ([SEND PACKET](send-packet)) ou lecture (**RECEIVE PACKET**) aura lieu.
