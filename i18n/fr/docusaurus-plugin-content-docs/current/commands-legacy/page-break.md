...
N'appelez pas **PAGE BREAK** avec la commande [PRINT SELECTION](print-selection) : dans ce cas, il est préférable d'utiliser les routines [Subtotal](subtotal) ou [BREAK LEVEL](break-level) avec leur paramètre optionnel pour générer des sauts de pages.
