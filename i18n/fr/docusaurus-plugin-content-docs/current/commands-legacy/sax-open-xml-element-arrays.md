...
Hormis la prise en charge de tableaux (cf. ci-dessous), cette commande est identique à [SAX OPEN XML ELEMENT](sax-open-xml-element). Reportez-vous à la description de cette commande pour le détail de son fonctionnement. 

**SAX OPEN XML ELEMENT ARRAYS** accepte des tableaux de type date, numérique, booléen et image comme paramètre(s) *tabValeursAttributs*. 4D effectue automatiquement les conversions nécessaires, vous pouvez paramétrer ces conversions à l’aide de la commande [XML SET OPTIONS](xml-set-options). 
