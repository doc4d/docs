---
id: end-sql
title: End SQL
slug: /commands/end-sql
displayed_sidebar: docs
---

<!--REF #_command_.End SQL.Syntax-->**End SQL**<!-- END REF-->
<!--REF #_command_.End SQL.Params-->
<div class="no-index">

| Ne requiert pas de paramètre |  |
| --- | --- |
</div>
<!-- END REF-->

<div class="no-index">
<details><summary>Historique</summary>

|Version|Changements|
|---|---|
|11 SQL|Créé|

</details>
</div>

## Description 

<!--REF #_command_.End SQL.Summary-->**End SQL** est un mot-clé indiquant dans l'éditeur de méthodes la fin d'une séquence de commandes SQL.<!-- END REF--> 

Une séquence d'instructions SQL doit être encadrée par les mot-clés [Begin SQL](begin-sql) et **End SQL**. Pour plus d'informations, reportez-vous à la description du mot-clé [Begin SQL](begin-sql). 

## Voir aussi 

[Begin SQL](begin-sql)  

## Propriétés

|  |  |
| --- | --- |
| Numéro de commande | 949 |
| Thread safe | yes |


