...
| Result         | Object | <- | Objet 4D View Pro |
<!-- END REF -->
...
|          | .success      | booléen | Vrai si l'export est réussi, Faux sinon.                   |
...
The `VP Font to object` utility command <!-- REF #_method_.VP Font to object.Summary -->returns an object from a font shorthand string<!-- END REF -->. Cet objet peut être utilisé pour définir ou lire les propriétés de la police via la notation objet.
...
| styleName  | Text    | -> | Nom du style                                      |
| sheet      | Integer | -> | Indice de la feuille (feuille courante si omis)   |
| Result     | Object  | <- | Objet feuille de style|<!-- END REF -->
...
| heure     | Real                                    | Valeur heure (en secondes) si la valeur est du type js |
...
| Paramètres |                 | Type  | Description                                                                                                                                                                                                                                                                           |
| ---------- | --------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| formula    |                 | Objet | Nom d'une méthode callback (ou méthode rétro-rappel) à lancer lorsque l'import est terminé. La méthode doit utiliser la commande [`Formula`](https://doc.4d.com/4dv19/help/command/en/page1597.html). Voir [Passing a callback method (formula)](#passing-a-callback-method-formula). |
| password   |                 | text  | Microsoft Excel uniquement (optionnel) - Mot de passe utilisé pour protéger un document Microsoft Excel.                                                                                                                                                                              |
| csvOptions |                 | Objet | options d'import csv                                                                                                                                                                                                                                                                  |
|            | range           | Objet | Plage de cellules contenant la première cellule dans laquelle les données seront saisies. Si la plage spécifiée n'est pas une plage de cellules, seule la première cellule de la plage est utilisée.                                                                                  |
...
| autoQuit                   | booléen         | True (default value) if the command must stop the formula execution when the [`On End URL Loading`](Events/onEndUrlLoading.md) or [`On URL Loading Error`](Events/onUrlLoadingError.md) events occur.If false, you must use the `CANCEL` or `ACCEPT` commands in the *onEvent* callback method.                                                                                                                                                                                                                                                                                                                                                              |
...
| pageBreak | booléen | Vrai pour insérer un saut de page avant la première colonne de la plage, sinon faux |
| visible   | booléen | Vrai si la colonne est visible, sinon faux                                          |
| resizable | booléen | Vrai si la colonne peut être redimensionnée, sinon faux                             |
...
The `VP SET CUSTOM FUNCTIONS` command <!-- REF #_method_.VP SET CUSTOM FUNCTIONS.Summary -->designates the 4D formulas that can be called directly from 4D View Pro formulas<!-- END REF -->. Les fonctions personnalisées n'étant pas stockées dans le document, la commande `VP SET CUSTOM FUNCTIONS` doit être exécutée dans l'événement formulaire `On Load`.
...
| heure     | Real                                     | Time value (in seconds) to put in the context |
...
| pageBreak | booléen | Vrai pour insérer un saut de page avant la première ligne de la plage, sinon faux |
| visible   | booléen | Vrai si la ligne est visible, sinon faux                                          |
| resizable | booléen | Vrai si la ligne peut être redimensionnée, sinon faux                             |
...
| heure     | Real                                     | Valeur heure (en secondes) à assigner à *rangeObj*                                                                                                                                       |
...
* Chaque sous-collection définit les valeurs des cellules de la ligne. Les valeurs peuvent être de type entier long, réel, booléen, texte, date, null ou objet. Si la valeur est un objet, elle peut avoir les propriétés suivantes :
 ...
 | heure     | Real                                     | Valeur heure (en secondes)            |
...
| Propriété                             | Type                    | Description                                                                                                                                                                                                                                            |
| ------------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| allowUserDragMerge                    | booléen                 | The drag merge operation is allowed (select cells and drag the selection to merge cells)                                                                                                                                                               |
| allowAutoCreateHyperlink              | booléen                 | Enables automatic creation of hyperlinks in the spreadsheet.                                                                                                                                                                                           |
| allowContextMenu                      | booléen                 | The built-in context menu can be opened.                                                                                                                                                                                                               |
| allowCopyPasteExcelStyle              | booléen                 | Styles from a spreadsheet can be copied and pasted to Excel, and vice-versa.                                                                                                                                                                           |
| allowDynamicArray                     | booléen                 | Enables dynamic arrays in worksheets                                                                                                                                                                                                                   |
| allowExtendPasteRange                 | booléen                 | Extends the pasted range if the pasted range is not enough for the pasted data                                                                                                                                                                         |
| allowSheetReorder                     | booléen                 | Sheet reordering is allowed                                                                                                                                                                                                                            |
| allowUndo                             | booléen                 | Undoing edits is allowed.                                                                                                                                                                                                                              |
| allowUserDeselect                     | booléen                 | Deselecting specific cells from a selection is allowed.                                                                                                                                                                                                |
| allowUserDragDrop                     | booléen                 | Drag and drop of range data is allowed                                                                                                                                                                                                                 |
| allowUserDragFill                     | booléen                 | Drag fill is allowed                                                                                                                                                                                                                                   |
| allowUserEditFormula                  | booléen                 | Formulas can be entered in cells                                                                                                                                                                                                                       |
| allowUserResize                       | booléen                 | Columns and rows can be resized                                                                                                                                                                                                                        |
| allowUserZoom                         | booléen                 | Zooming (ctrl + mouse wheel) is allowed                                                                                                                                                                                                                |
...
| backColor                             | Chaîne                  | A color string used to represent the background color of the area, such as "red", "#FFFF00", "rgb(255,0,0)", "Accent 5". The initial backgroundcolor is hidden when a backgroundImage is set.                                                          |
...
| calcOnDemand                          | booléen                 | Formulas are calculated only when they are demanded.                                                                                                                                                                                                   |
...
| customList                            | collectionMMM           | The list for users to customize drag fill, prioritize matching this list in each fill. Each collection item is a collection of strings. See on [GrapeCity's website](https://www.grapecity.com/spreadjs/docs/v13/online/AutoFillLists.html#b).         |
| cutCopyIndicatorBorderColor           | Chaîne                  | Border color for the indicator displayed when the user cuts or copies the selection.                                                                                                                                                                   |
| cutCopyIndicatorVisible               | booléen                 | Display an indicator when copying or cutting the selected item.                                                                                                                                                                                        |
...
| enableAccessibility                   | booléen                 | Accessibility support is enabled in the spreadsheet.                                                                                                                                                                                                   |
| enableFormulaTextbox                  | booléen                 | The formula text box is enabled.                                                                                                                                                                                                                       |
| grayAreaBackColor                     | Chaîne                  | A color string used to represent the background color of the gray area , such as "red", "#FFFF00", "rgb(255,0,0)", "Accent 5", and so on.                                                                                                              |
| highlightInvalidData                  | booléen                 | Invalid data is highlighted.                                                                                                                                                                                                                           |
| iterativeCalculation                  | booléen                 | Enables iterative calculation. See on [Grapecity's website](https://www.grapecity.com/spreadjs/docs/v14/online/calculating-iterative.html).                                                                                                            |
...
| newTabVisible                         | booléen                 | Display a special tab to let users insert new sheets.                                                                                                                                                                                                  |
...
| pasteSkipInvisibleRange               | booléen                 | Paste or skip pasting data in invisible ranges: <ul><li>False (default): paste data</li><li>True: Skip pasting in invisible ranges</li></ul>See [Grapecity's docs](https://www.grapecity.com/spreadjs/docs/v14/online/paste-skip-data-invisible-range.html) for more information on invisible ranges.                     |
...
| scrollbarMaxAlign                     | booléen                 | The scroll bar aligns with the last row and column of the active sheet.                                                                                                                                                                                |
| scrollbarShowMax                      | booléen                 | The displayed scroll bars are based on the entire number of columns and rows in the sheet.                                                                                                                                                             |
| scrollByPixel                         | booléen                 | Enable precision scrolling by pixel.                                                                                                                                                                                                                   |
| scrollIgnoreHidden                    | booléen                 | The scroll bar ignores hidden rows or columns.                                                                                                                                                                                                         |
...
| showDragDropTip                       | booléen                 | Display the drag-drop tip.                                                                                                                                                                                                                             |
| showDragFillSmartTag                  | booléen                 | Display the drag fill dialog.                                                                                                                                                                                                                          |
| showDragFillTip                       | booléen                 | Display the drag-fill tip.                                                                                                                                                                                                                             |
| showHorizontalScrollbar               | booléen                 | Display the horizontal scroll bar.                                                                                                                                                                                                                     |
...
| showVerticalScrollbar                 | booléen                 | Display the vertical scroll bar.                                                                                                                                                                                                                       |
| tabEditable                           | booléen                 | The sheet tab strip can be edited.                                                                                                                                                                                                                     |
| tabNavigationVisible                  | booléen                 | Display the sheet tab navigation.                                                                                                                                                                                                                      |
...
| tabStripVisible                       | booléen                 | Display the sheet tab strip.                                                                                                                                                                                                                           |
| tabStripWidth                         | number                  | Width of the tab strip when position is left or right. Default and minimum is 80.                                                                                                                                                                      |
| useTouchLayout                        | booléen                 | Whether to use touch layout to present the Spread component.                                                                                                                                                                                           |
