...
| Résultat   | 4D.EntitySelection |  <- | Entity selection matching the dataclass related to the given table                                   | <!-- END REF --> |
...
| Résultat        | 4D.EntitySelection |  -> | Entity selection including the added _entity_ or _entitySelection_ | <!-- END REF --> |
...
| Résultat        | 4D.EntitySelection |  <- | New entity selection with the result of intersection with logical AND operator | <!-- END REF --> |
...
| Résultat      | Real |  <- | Arithmetic mean (average) of entity attribute values (Undefined if empty entity selection) | <!-- END REF --> |
...
| Résultat   | Boolean   |  <- | True if the entity belongs to the entity selection, else False | <!-- END REF --> |
...
| ------------- | ---- | :-: | ----------------------------------------------------------------- | ---------------- |
| attributePath | Text |  -> | Chemin de l'attribut à utiliser pour le calcul                    |                  |
| Résultat      | Real |  <- | Number of non null _attributePath_ values in the entity selection | <!-- END REF --> |
...
| Résultat   | 4D.EntitySelection |  <- | Copy of the entity selection                     | <!-- END REF --> |
...
| Résultat      | Collection |  <- | Collection with only distinct values                                    | <!-- END REF --> |
...
| Résultat   | 4D.EntitySelection |  <- | Entity selection vide si exécutée avec succès, sinon entity selection contenant la ou les entité(s) non supprimée(s) | <!-- END REF --> |
...
| Résultat      | 4D.EntitySelection |  <- | New entity selection made up of entities from entity selection meeting the search criteria specified in _queryString_ or _formula_ | <!-- END REF --> |
...
| Résultat   | 4D.EntitySelection |  <- | New entity selection containing sliced entities (shallow copy) | <!-- END REF --> |
...
| ------------- | ---- | :-: | ---------------------------------------------- | ---------------- |
| attributePath | Text |  -> | Chemin de l'attribut à utiliser pour le calcul |                  |
| Résultat      | Real |  <- | Sum of entity selection values                 | <!-- END REF --> |
...
Si aucun paramètre de filtre n'est passé ou si le paramètre contient une chaîne vide ou "\*", tous les attributs sont extraits. Attributes with [kind](DataClassClass.md#attributename) property as "relatedEntity" are extracted with the simple form: an object with property __KEY (primary key). Les attributs dont la propriété "kind" est "relatedEntities" ne sont pas extraits.
